...
- **AC-4.1.12** — A keep-alive media element holds the audio session open while a run lasts
  ...
  - **Then** a looping media element starts alongside the run — playing generated silence until AC-4.1.15's render replaces it with the run itself, and never a shipped audio asset either way — so the page holds an active media session for as long as the run lasts, and it is stopped when playback stops
  - **And** it is inaudible while the screen is on, whichever of the two it is carrying
  - **Cases**:
