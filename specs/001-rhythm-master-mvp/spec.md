...
- **AC-15.1.8** — Fixed main-panel section order
  ...
  - **Then** the order is fixed top to bottom, below the pinned bar (the library toggle and quick navigation, AC-5.5.3): Pattern header → grid → play controls → Recipe strip (US-1.3) → pitch strip (US-2.2, Melodic only) → playback settings → edit controls → MIDI export (US-12.1) and other actions → family members (US-11.2, ≥768px only)
