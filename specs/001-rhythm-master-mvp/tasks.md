...
- [X] T209 **[test]** Tests for T208 — `tests/unit/core/timeline.test.js` (AC-4.4.13/1–/4), `tests/unit/core/pattern.test.js` (AC-4.4.13/5, `setSwingAmount` mechanics), `tests/unit/core/similarity.test.js` (AC-4.4.13/6), `tests/e2e/playback.spec.js` (AC-4.4.12/1–/3; AC-4.4.6/3 updated to the Pattern-wide overlay), `tests/unit/storage/storage.test.js` (`applyPlaybackTo` carries the amount; legacy per-group entries still apply).
