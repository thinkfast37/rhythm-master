...
- [X] T188 **[data]** Rename the 97 book-transcribed exercises and retag them — `data/seed-patterns.json`. Touches no AC; ids unchanged (renamed in place, US-5.3 overlays keyed by id are unaffected).
  ...
  **`WR Book` → `Exercise`** (singular, matching `Song`). Every `WR n-x` name — a direct index into the book — becomes `<Family> — <what distinguishes it>` in the em-dash form the library already uses (*Paradise — Intro*), e.g. `WR 9-G` → *Offbeat Eighths — Upbeat Drifting Earlier*. All 97 unique, clashing with nothing.
