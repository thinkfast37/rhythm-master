...
- [ ] T196 **[follow-up]** Back `storage/keyValue.js` with the native Preferences plugin inside the shell — `src/storage/keyValue.js`, `src/billing/`-style platform switch. Copy `ear-trainer` research.md **D-007** (`~/dev/ai-dev/ear-trainer`): one adapter, `localStorage` on the web and `@capacitor/preferences` on device, same key and `schemaVersion`, chosen by `Capacitor.isNativePlatform()`, debounced writes flushed on background. iOS may evict WKWebView `localStorage` under storage pressure (D-009, consequences), which is where a paying Musician's own Patterns live. Not part of T190–T195: a storage change is one the maintainer should choose knowingly (CLAUDE.md §5, "cannot be undone by a follow-up commit").
...
- [ ] T198 **[follow-up]** Configure the shell's audio session — `ios/App/App/Info.plist` (`UIBackgroundModes: audio`), a small Capacitor plugin or config setting `AVAudioSession` to `.playback` (iOS) and the equivalent audio focus / foreground handling on Android. Three practical wins, none needing a second audio engine: playback is not muted by the iPhone silent switch; playback survives the screen locking (phone in pocket); calls and headphone unplugs interrupt and resume sensibly. Verify on a device once Xcode is installed. Assessed 2026-08-18: a native audio *engine* was considered and parked — Web Audio on both platforms is CoreAudio / AAudio underneath, so DSP quality is not the limiter, and a native engine would sit outside every audio AC's proof (§2b); "better sound" would come from sampled content, which is D-003 territory and only worth reopening if the shell audibly disappoints in hand.

- [ ] T199 **[follow-up]** Check the percussive voices on a phone speaker — `src/audio/voices.js`. The three hits are pure sines at 200 / 370 / 620 Hz (`ACCENT_FREQUENCIES`); phone speakers roll off hard below ~500 Hz, so the Weak accent may nearly vanish on the built-in speaker, against the Constitution's "audibly distinct at practice tempos". If so, add harmonic content or a transient inside the existing engine — testable, no native path — and re-check AC-3.1.14 / AC-3.1.15.

- [X] T200 **[new capability]** Rate the open Pattern from its header — `specs/001-rhythm-master-mvp/spec.md` (AC-6.1.7), `specs/001-rhythm-master-mvp/plan.md` (P-028), `src/ui/library.js` (`renderStars` exported), `src/ui/controls.js` (the star row in `renderHeader`, wired to the existing `onRate`), `src/styles/tokens.css`. Implements AC-6.1.7/1–/4. Extends P-028. Requested by the maintainer 2026-08-22: the Rating should be settable and visible on the playback/composer panel, not only in the library review. Reuses the library's star control and the one `onRate` path, so a built-in Pattern's Rating still lands in the overlay store — the header is a second door onto the same Rating, never a second Rating (mirrors AC-5.3.10, which moved Tags onto the header for the same reason).

- [X] T201 **[test]** e2e tests for the header Rating — `tests/e2e/library.spec.js`. Covers AC-6.1.7/1–/4.

- [X] T202 **[spec revision]** Raise the tempo ceiling from 220 to 300 BPM — `specs/001-rhythm-master-mvp/spec.md` (AC-4.2.1 revised, dated parenthetical), `specs/001-rhythm-master-mvp/data-model.md` (§2 shape comment, §3 clamp row, §7 rule 8), `data/README.md`, `src/core/pattern.js` (`MAX_TEMPO`). Revises AC-4.2.1. Asked for by the maintainer 2026-08-22. The slider max and the validator both derive from `MAX_TEMPO`, so the constant is the whole code change; floor, default and presets unchanged — 300 is reached via the slider.

- [X] T203 **[test]** Update the tests the revised range touches — `tests/e2e/playback.spec.js` (the AC-4.2.1 test renamed to the criterion's title verbatim, asserts max 300), `tests/unit/core/pattern.test.js` (the AC-16.2.3 out-of-range sample moves from 300 to 301: it encoded the old AC-4.2.1 ceiling, which the revised AC contradicts — the §2a test-is-the-defect case, AC-4.2.1 settles it). Covers AC-4.2.1 as revised.

- [X] T204 **[new capability + spec revision]** Tempo and swing as per-Pattern playback settings — `specs/001-rhythm-master-mvp/spec.md` (AC-7.3.1 and AC-4.2.3 revised; AC-4.2.4 and AC-4.4.6 added), `specs/001-rhythm-master-mvp/plan.md` (P-011, P-013), `specs/001-rhythm-master-mvp/data-model.md` (`rm.overlays.v1` gains `tempo` and `swing`), `src/storage/overlays.js` (`setTempo`, `setSwing`, `applyPlaybackTo`), `src/main.js` (`onSwing` no longer guards shipped Patterns; `onTempo` remembers per shipped Pattern; load paths apply playback settings). Implements AC-4.2.4/1–/2, AC-4.4.6/1–/4; revises AC-7.3.1, AC-4.2.3. Extends P-011 and P-013. Asked for by the maintainer 2026-08-22: changing swing on a built-in forced "create a new rhythm" where tempo did not, and tempo/swing should be remembered per rhythm as playback settings — applied on load, saved on change. Spill approved same day: exports of a loaded shipped Pattern carry the playback tempo/swing heard; the library's `swing` Tag stays derived from shipped data.

- [X] T205 **[test]** Tests for T204 — `tests/e2e/playback.spec.js` (AC-4.2.4/1–/2, AC-4.4.6/1–/4), `tests/unit/storage/storage.test.js` (`applyPlaybackTo` mechanics).

- [X] T206 **[new capability]** Swing feel: choose the pulse level swing pairs at — `specs/001-rhythm-master-mvp/spec.md` (AC-4.4.7–AC-4.4.11 added; AC-4.4.2 and AC-4.4.5 clarified in dated parentheticals, titles unchanged), `specs/001-rhythm-master-mvp/plan.md` (P-013), `specs/001-rhythm-master-mvp/data-model.md` (`swingFeel` on the Pattern and in `rm.overlays.v1`; validation rule 11), `src/core/swing.js` (`SWING_FEELS`, `isValidSwingFeel`, `swungDelay`; `swungOffsets` gains a feel argument), `src/core/pattern.js` (`setSwingFeel`; validation), `src/core/timeline.js` (Quarters-feel Beat pairing; feel passed through to group offsets), `src/core/similarity.js` (feel joins the rhythm fingerprint when any swing amount is audible), `src/storage/overlays.js` (`setSwingFeel`, `applyPlaybackTo`), `src/main.js` (`onSwingFeel`), `src/ui/controls.js` (the Swing feel picker beside the swing slider). Implements AC-4.4.7/1–/3, AC-4.4.8/1–/3, AC-4.4.9/1–/3, AC-4.4.10/1–/5, AC-4.4.11/1–/2. Extends P-013.

  Asked for by the maintainer 2026-08-22: *"the way swing typically works is it delays the and of a beat. But sometimes, what I wanna do is if I were to play that same pattern in double time, so the two count of the beat would actually be the and of the one count, then the swing would actually work for it"* — and, for 16ths, *"you almost wanna go to halftime instead of double time, and then you'll get the swing feel."* Chosen over a slot-remapping double-time/half-time view (which would manufacture 32nd notes from a 4-Slot group) and over auto-detecting the level (ambiguous — a 4-Slot group sounding only Slots 1 and 3 could be either feel — and unstable while editing): no Slot moves, the feel only names the pulse the long–short pair sits on. Spill approved the same day: Quarters-feel pairing crosses Subdivision Group boundaries, so the feel is Pattern-level where the amount stays per group (AC-4.4.2 clarified), and duplicate identity gains the feel when audible (AC-4.4.11).

- [X] T207 **[test]** Tests for T206 — `tests/unit/core/swing.test.js` (AC-4.4.8/1–/3), `tests/unit/core/timeline.test.js` (AC-4.4.9/1–/3), `tests/unit/core/similarity.test.js` (AC-4.4.11/1–/2), `tests/e2e/playback.spec.js` (AC-4.4.7/1–/3, AC-4.4.10/1–/5), `tests/unit/storage/storage.test.js` (`applyPlaybackTo` carries the feel).

- [X] T208 **[new capability]** Open standalone from the Home Screen — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.15 added), `specs/001-rhythm-master-mvp/plan.md` (P-036), `index.html` (the `apple-mobile-web-app-*` metas and the `apple-touch-icon` link), `public/apple-touch-icon.png` (derived from the shell's `ios/App/App/Assets.xcassets/AppIcon.appiconset/AppIcon-512@2x.png`). Implements AC-15.1.15/1–/2. Extends P-036. Reported by the maintainer 2026-08-23: the Home-Screen entry opened as a plain bookmark in the device's default browser (Chrome, with an address bar) where Ear Trainer, which declares these metas, opens standalone. `viewport-fit=cover` and the safe-area padding in `tokens.css` were already in place; the metas and icon are the whole change.

- [X] T209 **[test]** Test for T208 — `tests/unit/ui/webapp-meta.test.js` (AC-15.1.15/1 reads the metas from `index.html`; AC-15.1.15/2 asserts the `apple-touch-icon` link and that the file it names exists).
- [X] T210 **[bug + spec revision]** The swing control goes Pattern-wide — `specs/001-rhythm-master-mvp/spec.md` (AC-4.4.12, AC-4.4.13 added; AC-4.4.2 and AC-4.4.6 clarified in dated parentheticals, titles unchanged), `specs/001-rhythm-master-mvp/plan.md` (P-013), `specs/001-rhythm-master-mvp/data-model.md` (`swingAmount` on the Pattern and in `rm.overlays.v1`; validation rule 12), `src/core/pattern.js` (`setSwingAmount` clears per-group overrides; `automaticTags` and `validate` read it), `src/core/timeline.js` (every straight group inherits `pattern.swingAmount` unless overridden, at every feel), `src/core/similarity.js` (fingerprint over the amounts each group actually plays), `src/storage/overlays.js` (`setSwingAmount`; `applyPlaybackTo`; the unused per-group `setSwing` removed), `src/main.js` (`onSwing` takes just the amount), `src/ui/controls.js` (one Pattern-wide slider, present when any Beat has a swingable group), `CLAUDE.md` (§2c's example no longer lists Swing among the M1B1-only controls). Implements AC-4.4.12/1–/3, AC-4.4.13/1–/6. Extends P-013.

  Reported by the maintainer 2026-08-22, by ear, on the deployed T206: *"the first bar works fine … but the second measure seems to ignore the swing feel."* The slider only ever wrote to Measure 1 Beat 1 (`onSwing`'s `measureIndex = 0, beatIndex = 0` defaults) — the exact partial-reachability shape §2c warns no gate can see. Fixed by inheritance rather than propagation: the amount lives on the Pattern, so a Measure added later — the maintainer's explicit follow-up case — or a re-subdivided Beat swings without any copying code path existing at all. Per-group amounts stay real as data overrides (AC-4.4.2's capability; Samba Break's authored swing still plays), and the slider clears them when moved so it never misreports what is heard.

  **One existing test changed (§2a), for encoding the old mechanism rather than the criterion.** *AC-4.4.6/3* asserted the overlay held `swing: { "0.0.0": 25 }`; the criterion says the remembered swing lives in the overlay store, which it still does — as the Pattern-wide `swingAmount` the control now sets. The assertion follows the mechanism; the criterion's own claims (overlay store, shipped data unchanged) are unchanged, and AC-4.4.6's parenthetical records the shift.

- [X] T211 **[test]** Tests for T210 — `tests/unit/core/timeline.test.js` (AC-4.4.13/1–/4), `tests/unit/core/pattern.test.js` (AC-4.4.13/5, `setSwingAmount` mechanics), `tests/unit/core/similarity.test.js` (AC-4.4.13/6), `tests/e2e/playback.spec.js` (AC-4.4.12/1–/3; AC-4.4.6/3 updated to the Pattern-wide overlay), `tests/unit/storage/storage.test.js` (`applyPlaybackTo` carries the amount; legacy per-group entries still apply).

- [X] T212 **[bug + spec addition]** The armed pitch-strip chip stays legible under hover — `specs/001-rhythm-master-mvp/spec.md` (AC-2.2.18 added), `specs/001-rhythm-master-mvp/plan.md` (P-016), `src/styles/tokens.css` (the strip's hover rule no longer applies to a control that is `aria-pressed`, so the armed fill and its dark ink stay together). Implements AC-2.2.18. Extends P-016. Reported by the maintainer 2026-08-23: tapping a degree in the strip turned it black-on-black. `.pitch-strip button:hover:not(:disabled)` outranks `.pitch-strip button[aria-pressed='true']` in specificity, so the armed chip kept its near-black ink while the hover ground replaced the brand fill — permanently on a touchscreen, where hover sticks after the tap.

- [X] T213 **[test]** Test for T212 — `tests/e2e/melodic.spec.js` (AC-2.2.18: hover the armed degree and assert the armed fill still paints).

- [X] T214 **[spec revision + rebuild]** The pitch strip goes chromatic — `specs/001-rhythm-master-mvp/spec.md` (AC-2.2.4 revised from diatonic-plus-accidental-mode to twelve chromatic chips, gaining Cases /1–/3; AC-2.2.16/1–/2 revised to drop the armed accidental; AC-2.2.18's Given loses the accidental group), `specs/001-rhythm-master-mvp/data-model.md` (§3's pitch note re-worded for the chromatic strip; the token vocabulary itself unchanged), `src/core/pitch.js` (`DEFAULT_DEGREES`, `EXTENDED_DEGREES`, `ACCIDENTALS` retired — the strip’s vocabulary now comes from `src/core/scales.js`), `src/ui/controls.js` (`renderPitchStrip` rebuilt: twelve chromatic chips, no accidental group, no extend control), `src/main.js` (`onExtendDegrees` and `degreesExtended` removed). Implements AC-2.2.4/1–/3, revised AC-2.2.16/1–/2. Extends P-016. Asked for by the maintainer 2026-08-23: in F, armed degree 4 read `Bb4` while ♮ showed pressed, and ♯ relabelled the whole strip — the accidental-as-mode design confused the instrument it was built for. Spill approved same day (chromatic strip + scale picker, AC-2.2.4 reversed, 9–15 extension dropped in favour of the octave stepper).

- [X] T215 **[test]** Tests for T214 — `tests/e2e/melodic.spec.js` (AC-2.2.4/1–/3 named tests; AC-2.2.14/5, AC-2.2.15/1–/2, AC-2.2.16/1–/2, AC-2.2.18 re-pointed at chromatic chips instead of the accidental group), `tests/e2e/responsive.spec.js` (extend-control step removed), `tests/unit/core/pitch.test.js` (retired-export assertions removed; token round-trips kept).

- [X] T216 **[new capability]** Choose a scale — `specs/001-rhythm-master-mvp/spec.md` (US-2.5, AC-2.5.1–AC-2.5.5 added), `specs/001-rhythm-master-mvp/plan.md` (P-040), `specs/001-rhythm-master-mvp/data-model.md` (`scale` field; validation rule 13), `src/core/scales.js` (the catalogue — seven church modes, five pentatonic modes, both blues scales, harmonic and melodic minor, adopted from fret-navigator and extended with the three pentatonic modes it lacked; `chromaticStrip(scaleId)` labels the twelve chips by the scale's own spelling), `src/core/pattern.js` (validation rule 13), `src/ui/controls.js` (the scale picker on the pitch strip; in-scale chip marking), `src/main.js` (`onScale`; Sound Mode switch adds/removes `scale` beside `key`), `src/styles/tokens.css` (in-scale/out-of-scale chip styling, by more than colour), `data/seed-patterns.json` (every shipped Melodic Pattern stamped `"scale": "ionian"` — the maintainer's explicit instruction), `tools/validate-seed.js` (rule 13 for the seed). Implements AC-2.5.1/1–/4, AC-2.5.2/1–/3, AC-2.5.3, AC-2.5.4/1–/5, AC-2.5.5. Adds P-040.

- [X] T217 **[test]** Tests for T216 — `tests/e2e/melodic.spec.js` (AC-2.5.1/1–/4, AC-2.5.2/1–/3, AC-2.5.3, AC-2.5.4/1–/5, AC-2.5.5 — all UI-level, so all proven against the DOM), `tests/unit/core/scales.test.js` (catalogue and strip mechanics the e2e tests stand on), `tests/unit/core/pattern.test.js` (validation rule 13).

- [X] T218 **[bug + spec addition]** Operating a control neither steals focus nor moves the view — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.16/1–/3 added), `specs/001-rhythm-master-mvp/plan.md` (P-036), `src/ui/controls.js` (panels rebuild through a focus-preserving swap: the input/select being operated is kept alive in place and patched, other focused controls are re-focused by identity after the rebuild), `src/ui/responsive.js` (`AUTOSCROLL_GRACE_MS`), `src/main.js` (main-panel interaction stamps a timestamp; the sounding-Measure autoscroll stands down within the grace window; the focused control's on-screen position is re-anchored after a render changes the height above it). Implements AC-15.1.16/1–/3. Extends P-036. Reported by the maintainer 2026-08-23: every tap on the swing slider or the Sound select dropped focus and, during playback, snapped the view back to the sounding Measure.

- [X] T219 **[test]** Tests for T218 — `tests/e2e/responsive.spec.js` (AC-15.1.16/1–/3 named tests, all UI-level and DOM-proven).

- [X] T220 **[new capability]** Opening another Pattern during playback switches playback to it — `specs/001-rhythm-master-mvp/spec.md` (AC-4.1.8 added), `specs/001-rhythm-master-mvp/plan.md` (P-010), `src/main.js` (`loadPattern` restarts the running transport with the newly loaded Pattern, the same restart path a tempo change uses). Implements AC-4.1.8/1–/3. Extends P-010. Requested by the maintainer 2026-08-23: opening a Pattern from the library mid-playback left the previous Pattern sounding, so the panel showed one rhythm while the transport played another. Living in `loadPattern` means library clicks, Prev/Next, and every other load path switch the same way; when the transport is stopped, nothing starts (FR-010).

- [X] T221 **[test]** Tests for T220 — `tests/e2e/playback.spec.js` (AC-4.1.8/1–/3 named tests, DOM-driven through the library list).

- [X] T222 **[bug + spec addition]** A Pattern with no straight-feel group says swing doesn't apply — `specs/001-rhythm-master-mvp/spec.md` (AC-4.4.14 added; AC-4.4.7 clarified in a dated parenthetical, title unchanged), `specs/001-rhythm-master-mvp/plan.md` (P-013), `src/ui/controls.js` (`renderSwing`: presence decided by "any straight-feel group" rather than "any even-slot straight group"; on an all-triplet Pattern both controls give way to one explanatory note), `src/styles/tokens.css` (`.swing-note`). Implements AC-4.4.14/1–/3. Extends P-013. Reported by the maintainer 2026-08-23 from the deployed app: on the all-triplet Afrobeat Triplet Clave the slider was absent while the feel picker stood alone, which read as a loading failure. The old even-slot test also wrongly hid the control on all-Undivided Patterns, which the Quarters feel (AC-4.4.9, built for exactly that shape) can swing.

- [X] T223 **[test]** Tests for T222 — `tests/e2e/playback.spec.js` (AC-4.4.14/1–/3 named tests, all UI-level and DOM-proven, the /3 case covering both a mixed and an all-Undivided Pattern).

- [X] T224 **[bug + spec addition]** Arming a pitch disarms the armed Recipe — `specs/001-rhythm-master-mvp/spec.md` (AC-1.3.11/5 added, with a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-007), `src/main.js` (`onArmDegree` and `onArmOctave` clear `armedRecipe`, so the Recipe brush yields to the pitch strip). Implements AC-1.3.11/5. Extends P-007. Reported by the maintainer 2026-09-05: in a Melodic Pattern, changing the octave and picking a note, then tapping a Slot, changed the Beat's subdivision instead of stamping the pitch — a Recipe armed earlier still owned every grid tap, and nothing on the pitch strip ended that state.

- [X] T225 **[test]** Test for T224 — `tests/e2e/melodic.spec.js` (AC-1.3.11/5 named test: arm a Recipe, step the octave and arm a degree, then tap a note band and assert the pitch stamps and the Beat's Recipe is unchanged).

- [X] T226 **[new capability]** The grid always states its active brush — `specs/001-rhythm-master-mvp/spec.md` (AC-1.3.12/1–/4 added), `specs/001-rhythm-master-mvp/plan.md` (P-007), `src/ui/controls.js` (the Recipe strip's hint becomes a brush indicator: Subdivision with the armed Recipe's label while one is armed; Note with the armed degree and its note name in Melodic mode otherwise; the accent default in Percussive mode; `data-brush` carries the state), `src/styles/tokens.css` (indicator styling keyed off `data-brush`). Implements AC-1.3.12/1–/4. Extends P-007. Asked for by the maintainer 2026-09-05, with T224's fix: the strips are exclusive brushes switched by tapping, so a label must always say which is in hand — the old hint only ever described subdivision mode.

- [X] T227 **[test]** Tests for T226 — `tests/e2e/grid.spec.js` (AC-1.3.12/1–/4 named tests, all UI-level and DOM-proven; /4 crosses the switch in both directions via the pitch strip and the Recipe strip).

- [X] T228 **[new capability]** Edits during playback are heard from the next pass — `specs/001-rhythm-master-mvp/spec.md` (AC-4.1.9 added), `specs/001-rhythm-master-mvp/plan.md` (P-010), `src/audio/scheduler.js` (`update(nextPattern, nextSettings)`: the running transport re-derives its timeline, beat grid and loop duration at the next pass boundary, re-anchoring the audio-clock origin there so event times stay products of an absolute origin, never accumulated sums — FR-009; the loop counter carries across the re-anchor), `src/main.js` (`apply`, `undo`, `onKey`, `onScale`, `onSoundMode` feed the running transport the edited Pattern). Implements AC-4.1.9/1–/5. Extends P-010. Asked for by the maintainer 2026-09-07: edits made while playing were not heard until Stop then Play; the pickup he asked for is "the next time the cycle plays through", not an immediate restart.

- [X] T229 **[test]** Tests for T228 — `tests/e2e/playback.spec.js` (AC-4.1.9/1–/5 named tests, driving the real transport and asserting the pass-boundary pickup through the `_snapshot` seam and the DOM).

- [X] T230 **[bug + spec addition]** Play recovers audio the device took away — `specs/001-rhythm-master-mvp/spec.md` (AC-4.1.10 added, decomposing FR-011's recovery clause), `src/audio/context.js` (`resume()` handles the `interrupted` state iPadOS Safari reports, not only `suspended`, and replaces a context that stays dead after a resume attempt), `src/audio/scheduler.js` (`start` takes the context `resume()` returns and rebuilds the master gain when the context was replaced; `src/audio/nodes.js` already rebuilds per context). Implements AC-4.1.10/1–/3. Extends P-010. Reported by the maintainer 2026-09-07 from the iPad: returning from another app left the sound gone until the app was closed and reopened.

- [X] T231 **[test]** Tests for T230 — `tests/unit/audio/context.test.js` (AC-4.1.10/1–/2 against a fake AudioContext), `tests/e2e/playback.spec.js` (AC-4.1.10/3: background, force the injected context wrapper to report `interrupted`, return, press Play, and assert the cursor runs again).

- [X] T232 **[new capability]** The Key is chosen on the pitch strip — `specs/001-rhythm-master-mvp/spec.md` (AC-2.2.19 added), `specs/001-rhythm-master-mvp/plan.md` (P-016), `src/ui/controls.js` (the Key picker moves from `renderSound` in the Edit accordion to the pitch strip, beside the scale picker; behaviour and handler unchanged). Implements AC-2.2.19/1–/3. Extends P-016. Asked for by the maintainer 2026-09-07: "the key, when I'm in Melodic — I really think that should be in the notes section, because I'm up and hunting around trying to find it."

- [X] T233 **[test]** Tests for T232 — `tests/e2e/melodic.spec.js` (AC-2.2.19/1–/3 named tests, DOM-proven).

- [X] T234 **[spec revision + new capability]** Tempo presets to the ceiling, and exact tempo entry — `specs/001-rhythm-master-mvp/spec.md` (AC-4.2.5, AC-4.2.6 added; AC-4.2.1's parenthetical revised — "300 is reached via the slider" superseded), `specs/001-rhythm-master-mvp/plan.md` (P-011), `src/ui/controls.js` (`TEMPO_PRESETS` gains 240, 260, 280, 300; a clamped number-entry field beside the tempo slider), `src/styles/tokens.css` (`.value-entry`). Implements AC-4.2.5/1–/3, AC-4.2.6/1–/2; revises AC-4.2.1's parenthetical. Extends P-011. Asked for by the maintainer 2026-09-07; preset values chosen with the maintainer the same day.

- [X] T235 **[test]** Tests for T234 — `tests/e2e/playback.spec.js` (AC-4.2.5/1–/3, AC-4.2.6/1–/2 named tests, DOM-proven).

- [X] T236 **[new capability + spec revision]** Swing presets, exact swing entry, and touch-drag stability — `specs/001-rhythm-master-mvp/spec.md` (AC-4.4.15, AC-4.4.16 added; AC-15.1.16 revised, gaining /4 and /5), `specs/001-rhythm-master-mvp/plan.md` (P-013, P-036), `src/ui/controls.js` (swing preset row 0/15/25/33/50 and a clamped number-entry field, both routed through the one `onSwing` path; the feel picker and the amount on their own labelled lines; the operated control tracked by pointer as well as focus — `heldControl()` — so the keep-alive rebuild works on a touchscreen that never focuses a slider), `src/main.js` (the AC-15.1.16/3 scroll anchor falls back to the pointer-held control when nothing holds focus), `src/styles/tokens.css` (swing rows and presets). Implements AC-4.4.15/1–/3, AC-4.4.16/1–/3, AC-15.1.16/4–/5. Extends P-013 and P-036. Reported and asked for by the maintainer 2026-09-07: "as soon as I touch the swing thing, it refocuses the entire screen" — iPadOS Safari moves no focus onto a range input, so every AC-15.1.16 protection, keyed off `document.activeElement`, silently stood down under touch.

- [X] T237 **[test]** Tests for T236 — `tests/e2e/playback.spec.js` (AC-4.4.15/1–/3, AC-4.4.16/1–/3 named tests), `tests/e2e/responsive.spec.js` (AC-15.1.16/4–/5 named tests, driving the slider by pointer events without focus).

- [X] T238 **[bug + spec revision]** A tap on a button lands while playback renders are streaming — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.16 revised, gaining /6), `specs/001-rhythm-master-mvp/plan.md` (P-036), `src/ui/controls.js` (the pointer-held tracking covers buttons too, resolved via `closest()` so a tap landing on a span inside a button still counts; the keep-alive rebuild preserves the held control whatever its tag, and a kept-alive button's label is synced from the fresh render). Implements AC-15.1.16/6. Extends P-036. Reported by the maintainer 2026-09-07: at 200 BPM the Stop button "does not seem to work easily", working better at slow tempos — playback re-renders on every sounding event, so at fast tempos a render replaced the Stop button inside nearly every tap's down-to-up window and the browser never dispatched the click.

- [X] T239 **[test]** Test for T238 — `tests/e2e/responsive.spec.js` (AC-15.1.16/6 named test: hold the Stop button by pointer through streaming playback renders, assert its node survives, then release and assert the tap stops the transport).

- [X] T240 **[docs]** A comprehensive guide to how this project uses Spec Kit — `docs/speckit-guide.md` (new: the artifact map with what each file contains, the command lifecycle step by step, the local scaffolding — CLAUDE.md, spec-trace, coverage:ac, check:unwired, pattern-intake, the CI gates — and walk-throughs of how each kind of change is made and landed, with worked examples from this log), `README.md` (Documentation table row and a Development-workflow pointer to the guide). Asked for by the maintainer 2026-09-07: the workflow was built interactively and never written down as a map — "how SpecKit is used step by step, where the artifacts are, what they contain, how we've modified or extended it, and how new features are added and bugs are addressed". Documentation only: no AC, plan item, or code touched, so the traceability chain is unchanged.

- [X] T241 **[docs]** The Spec Kit guide teaches by real excerpt — `docs/speckit-guide.md` (excerpts from the actual artifacts woven in at each concept: Principle I's numerator rule and the versioning policy from the constitution; AC-1.1.9's Given/When/Then, AC-1.1.5's Cases and its dated parenthetical from spec.md; P-005/P-007 rows from plan.md's matrix; D-001 with a rejected alternative from research.md; the generated matrix's header and AC-1.1.9's live HIGH · WRONG TEST row; the real wrong-shape test name beside the real verbatim one; the waiver and unwired-baseline note fields with the removeMeasure entry; T224/T225 as a complete worked log entry; the 3.5.0 Sync Impact excerpt — each chosen for stability, marked as a dated snapshot where status is live). Asked for by the maintainer 2026-09-07, on reading T240's guide: "use excerpts from actual files… pick things that are unlikely to change but are still useful to explain a concept — I'm very example and concept driven." Documentation only; the traceability chain is unchanged.

- [X] T242 **[bug]** Playback survives a throttled timer on TV browsers — `src/audio/scheduler.js` (the lookahead becomes an adaptive floor: a tick arriving after a gap the current lookahead could not cover widens it, capped at 1.5s, and a stall that leaves events past the horizon shifts the origin forward by the deficit so the starved stretch stays a pause instead of every missed event sounding at once; devices whose timers keep up never widen anything and behave exactly as before). Fixes AC-4.1.1 on throttled browsers. Reported by the maintainer 2026-09-07, practising on a Hisense TV's built-in browser: "it pauses for too long or it speeds up suddenly" — the TV clamps `setInterval` far past the 25ms poll, the 0.2s lookahead starves, and Web Audio sounds past-scheduled events immediately.

- [X] T243 **[test]** Tests for T242 — `tests/unit/audio/scheduler.test.js` (new: AC-4.1.1 named tests driving the transport with a fake audio clock and stalled ticks — no event sounds in the past, post-stall events keep the tempo grid, the lookahead widens after a stall and stays at 0.2s under timely polling).

- [X] T244 **[spec revision + new capability]** Tempo and swing carry from the Pattern just left — `specs/001-rhythm-master-mvp/spec.md` (AC-4.2.3 rewritten with Cases /1–/5; AC-4.4.17 added), `specs/001-rhythm-master-mvp/plan.md` (P-011, P-013), `specs/001-rhythm-master-mvp/data-model.md` (§6 `rm.settings.v1` gains `lastSwingAmount` and `lastSwingFeel`), `src/core/playback-defaults.js` (new: `carriedPlaybackFor` resolves overlay → the Pattern's own authored value → the carry, returning only the fields the carry governs; `playbackInEffect` reports what a loaded Pattern is sounding at; `NO_CARRY` is the fresh-install starting point), `src/storage/settings.js` (the three `last*` keys, defaulted from `NO_CARRY`), `src/main.js` (`withCarriedPlayback` applies the carry to the loaded copy only; `loadPattern` records what is in effect; `onSwing` and `onSwingFeel` record theirs as `onTempo` already did). Implements AC-4.2.3/1–/5, AC-4.4.17/1–/5. Extends P-011 and P-013. Asked for by the maintainer 2026-09-12: "a lot of rhythms I practice in a row need to be sped up, but I always end up having to start off at eighty and then changing it." `lastTempo` had been written on every tempo change since the MVP and never read back, and could not have helped if it were — 172 of the 208 shipped Patterns carry the 80 BPM default as an authored tempo, so AC-4.2.3's "no tempo of its own saved" never described one. The maintainer chose, when asked: the tempo **in effect** carries rather than the last one set by hand; an authored tempo that differs from 80 still wins; and swing and the swing feel carry alongside the tempo.

- [X] T245 **[test]** Tests for T244 — `tests/unit/core/playback-defaults.test.js` (new: the pure resolver, each case named for its criterion with a `: the resolver` qualifier), `tests/e2e/playback.spec.js` (AC-4.2.3/1–/5 and AC-4.4.17/1–/5 named tests driving the real open path: carrying onto a shipped Pattern at the default, a remembered tempo and a remembered swing outranking the carry, an authored tempo outranking it, a carry from a Pattern whose controls were never touched, per-group swing data surviving a carried amount, reload persistence, the fresh-install defaults, and the `swing` Tag staying off both the header and the library row). AC-4.2.3's former test — named "the last-used tempo persists across a reload", for a criterion that then had no Cases — is folded into AC-4.2.3/5, which asserts the same persistence plus the carry it exists to feed.

- [X] T246 **[new capability + spec revision]** Prev/Next is pinned to the top of the main panel — `specs/001-rhythm-master-mvp/spec.md` (AC-5.5.3 added; AC-15.1.8's section order revised, quick navigation leaving the ordered list for the pinned bar), `specs/001-rhythm-master-mvp/plan.md` (P-027, P-036), `src/main.js` (the nav moves out of the foot of the panel into a new `.main-top-bar` alongside the library toggle), `src/styles/tokens.css` (`.main-top-bar` is the sticky element now, opaque since the grid scrolls under it; `.library-toggle` keeps its appearance and loses its own stickiness; `.pattern-nav` sits at the bar's trailing edge; the bar stops being sticky while the mobile drawer is open, since `position: sticky` is a stacking context whatever its `z-index` and the toggle goes `fixed` above the scrim from inside it). Implements AC-5.5.3/1–/3; revises AC-15.1.8. Extends P-027 and P-036. Asked for by the maintainer 2026-09-12: "right now it's always at the bottom — sometimes I just scroll to the bottom. Ideally that next and previous control is not requiring me to scroll." One pinned control was chosen over one at each end, so there are not two bars to keep in agreement.

- [X] T247 **[test]** Tests for T246 — `tests/e2e/responsive.spec.js` (AC-15.1.8's named test follows the revised AC: quick navigation leaves the ordered list and the pinned bar is excluded from it, as the library toggle already was), `tests/e2e/library.spec.js` (AC-5.5.3/1–/3 named tests: Prev/Next measured inside the viewport with the main panel scrolled to its foot and still working from there, exactly one Prev/Next control in the panel and it is in the pinned bar, and the same reachability at 390px with an accordion opened to make the panel scroll).

- [X] T248 **[test defect]** AC-15.1.16/2's test no longer races the renders it is testing — `tests/e2e/responsive.spec.js` (the setup step focuses the swing slider before scrolling to it, rather than after). The test failed once in CI on T246's merge — `locator.scrollIntoViewIfNeeded: Element is not attached to the DOM` — and passes locally at 5/5 and 3/3 under parallel load. The defect is in the test, not the code, and AC-15.1.16/1 is what settles it: the app preserves the DOM node of the control **being operated**, deliberately not one merely being reached for, so scrolling to a slider that nothing holds races the playback renders by design. Ordering only: no assertion is relaxed, no case narrowed, and the hands-on loop that proves the criterion is untouched.
- [X] T249 **[new capability]** Melody builder: chord progressions layered on a rhythm — the core and the data — `specs/001-rhythm-master-mvp/spec.md` (US-2.6 added, AC-2.6.1–AC-2.6.10; AC-15.1.8 revised — the chord strip and the harmony controls join the fixed section order, Melodic-only like the pitch strip; dated clarifications on US-2.2's preamble, AC-2.2.8, AC-11.1.2 and AC-12.1.2), `specs/001-rhythm-master-mvp/plan.md` (P-041), `specs/001-rhythm-master-mvp/data-model.md` (§1 `harmony`, §3 chord-tone Pitch, §4 derived chord in force, §7 rules 14–16), `specs/001-rhythm-master-mvp/research.md` (D-011), `src/core/harmony.js` (new: quality catalogue, progression catalogue, diatonic spelling from Key and scale, chord-tone resolution and naming, the chord in force per pass and Measure, the harmonic cycle length, the arpeggio fill, and every harmony mutation), `src/core/pitch.js` (`spellFromLetter` shared with harmony), `src/core/timeline.js` (`buildTimeline(pattern, pass)` resolves chord tones through the chord in force; events carry `pass` and `chordIndex`), `src/core/pattern.js` (validation rules 14–16; Percussive strips harmony), `src/core/similarity.js` (pitch fingerprint includes roles and the progression), `src/audio/scheduler.js` (the timeline is rebuilt for each pass a progression makes different), `src/export/midi.js` (the file spans the harmonic cycle), `src/export/submit.js` (a submission carries the progression), `tools/validate-seed.js` (rules 14–16 mirrored). Implements AC-2.6.1/2–/6, AC-2.6.2/2–/5, AC-2.6.3, AC-2.6.4, AC-2.6.6/2–/6, AC-2.6.8/2, AC-2.6.8/4, AC-2.6.8/5, AC-2.6.9, AC-2.6.10. Asked for by the maintainer 2026-09-12; spill approved the same day: chord-tone roles on the strip, a timeline parameterised by pass, MIDI exporting the full cycle, duplicate detection including harmony, non-heptatonic scales spelled from the parallel Ionian or Aeolian, the 8-Measure cap kept.
- [X] T250 **[new capability]** Melody builder: the harmony controls, the chord strip and the grid — `src/ui/controls.js` (`renderHarmony`: progression picker, change setting, per-chord root and quality editors with add and remove, fill order and Fill; role chips on the pitch strip naming the note under the chord in force), `src/ui/grid.js` (`renderChordStrip`: every chord with numeral and name, Now and Next marked by word; Measure headers name their chord when it changes every Measure; a chord-tone note band shows role and note for the current pass), `src/main.js` (harmony handlers, all guarded on shipped Patterns; the armed pitch may be a role; the chord strip mounted above the grid and the harmony section under the pitch strip; the current pass reaches the grid), `src/styles/tokens.css` (chord strip, harmony section, role chips), `tests/e2e/responsive.spec.js` (AC-15.1.8's expected order gains the two sections). Implements AC-15.1.8 as revised, AC-2.6.1/1, AC-2.6.2/1, AC-2.6.2/6, AC-2.6.5, AC-2.6.6/1, AC-2.6.6/7, AC-2.6.7, AC-2.6.8/1, AC-2.6.8/3. Extends P-041.
- [X] T251 **[test]** Tests for T249 — `tests/unit/core/harmony.test.js` (new: AC-2.6.1/2–/6, AC-2.6.2/2–/5, AC-2.6.3/1–/3, AC-2.6.4/1–/5, AC-2.6.6/2–/6 named tests over the pure core), `tests/unit/core/similarity.test.js` (AC-2.6.9/1–/2), `tests/unit/core/pattern.test.js` (AC-2.6.8/2, AC-2.6.8/4), `tests/unit/export/export.test.js` (AC-2.6.4/6, AC-2.6.8/5, AC-2.6.10/1–/3), `tests/unit/audio/scheduler.test.js` (AC-2.6.3/4).
- [X] T252 **[test]** Tests for T250 — `tests/e2e/harmony.spec.js` (new: AC-2.6.1/1, AC-2.6.2/1, AC-2.6.2/6, AC-2.6.5/1–/6, AC-2.6.6/1, AC-2.6.6/7, AC-2.6.7/1–/6, AC-2.6.8/1, AC-2.6.8/3 named tests, DOM-proven; the Now and Next marks driven by the real transport).
- [X] T253 **[spec defect + bug]** Choosing a progression re-reads the notes already there — `specs/001-rhythm-master-mvp/spec.md` (AC-2.6.1/7 and AC-2.6.5/7 added), `specs/001-rhythm-master-mvp/plan.md` (P-041), `src/core/harmony.js` (`setProgression` converts each fixed degree that is a member of the first chord into that role, other degrees untouched; `degreeAsTone` shared with the strip), `src/main.js` (the armed pitch is re-read the same way when a progression is chosen), `src/ui/controls.js` (a one-line hint in the harmony section while no Slot holds a role), `src/styles/tokens.css` (`.harmony-hint`). Implements AC-2.6.1/7, AC-2.6.5/7. Extends P-041. Reported by the maintainer 2026-09-12 on first use: "it sounded like it was literally just playing C the entire time even though it was saying it was playing the one four five" — every Slot held degree 1, a fixed note, and nothing said the progression only moves chord tones.
- [X] T254 **[test]** Tests for T253 — `tests/unit/core/harmony.test.js` (AC-2.6.1/7 named tests: tonics become Roots, a non-member degree stays fixed, the octave is preserved, and None then a fresh choice round-trips), `tests/e2e/harmony.spec.js` (AC-2.6.5/7 named test; the tests that turned Slots on after choosing a progression now expect the Root role the re-read armed pitch stamps).
- [X] T255 **[spec revision + new capability]** The arpeggio is a setting the notes follow, not a one-shot Fill — `specs/001-rhythm-master-mvp/spec.md` (AC-2.6.6 rewritten with Cases /1–/8; AC-2.6.5/7, AC-2.6.8/1, AC-2.6.9/1 retitled; AC-2.6.7/7 added; AC-2.6.4 clarified), `specs/001-rhythm-master-mvp/plan.md` (P-041), `specs/001-rhythm-master-mvp/data-model.md` (§1 `harmony.arpeggio`, §4 the dealt role, §7 rule 14), `specs/001-rhythm-master-mvp/research.md` (D-011 amended), `src/core/harmony.js` (`ARPEGGIOS`, `setArpeggio`, `arpeggioDeal` — a continuous deal over the sounding Slots, derived and never stored — and `soundingPitch`; `fillChordTones` removed; `clearHarmony` bakes the dealt roles), `src/core/timeline.js` (resolves the dealt role), `src/core/pattern.js` and `tools/validate-seed.js` (rule 14), `src/core/similarity.js` (the fingerprint carries the arpeggio), `src/main.js` (`onArpeggio`; Fill handlers removed), `src/ui/controls.js` (the Arpeggio picker under the progression; the pitch strip hides its chips and says the notes follow the arpeggio; the hint names the setting), `src/ui/grid.js` (note bands show the dealt role and are inert while dealing), `src/styles/tokens.css`. Implements AC-2.6.6/1–/8, AC-2.6.7/7; revises AC-2.6.5/7, AC-2.6.8/1, AC-2.6.9/1. Extends P-041. Asked for by the maintainer 2026-09-12: "what I wanted was arpeggios … c e g c e g over and over … I don't think you implemented the way I was expecting."
- [X] T256 **[test]** Tests for T255 — `tests/unit/core/harmony.test.js` (AC-2.6.6/2–/7 named tests over the pure deal; AC-2.6.1/6 covers the dealt bake), `tests/unit/core/similarity.test.js` (AC-2.6.9/1 retitled, arpeggio variant), `tests/e2e/harmony.spec.js` (AC-2.6.6/1, /7, /8, AC-2.6.7/7 named tests; AC-2.6.5/7 and AC-2.6.8/1 retitled and extended; the Fill tests retired with the control).
- [X] T257 **[spec revision + new capability]** Fill patterns as step sequences: octave shapes, drones and scale walks — `specs/001-rhythm-master-mvp/spec.md` (AC-2.6.6/1 and /2 revised, /9–/12 added), `specs/001-rhythm-master-mvp/plan.md` (P-041), `specs/001-rhythm-master-mvp/data-model.md` (§1, §4), `specs/001-rhythm-master-mvp/research.md` (D-011 amended), `src/core/harmony.js` (a step is a chord tone at an octave, a drone on the Key's tonic or the chord's root, or a step of a scale rooted on the chord; `ARPEGGIOS` grows to twenty-three entries in three groups, each a step sequence built from the progression's roles and the walk's scale; `arpeggioDeal(pattern, pass)` restarts at every chord change; `resolveStep`, `stepLabel`, `stepName`, `stepAsDegree`), `src/core/timeline.js` (resolves dealt steps for the pass), `src/ui/grid.js` (the band names the step), `src/ui/controls.js` (the picker is grouped), `tools/validate-seed.js` (the id list). Implements AC-2.6.6/9–/12; revises AC-2.6.6/1–/2. Extends P-041. Asked for by the maintainer 2026-09-12: the tonic drone ("I'll play c, then a high octave c, then e, then a high octave c…") and scale walks from the chord root in major, minor and the pentatonics.
- [X] T258 **[test]** Tests for T257 — `tests/unit/core/harmony.test.js` (AC-2.6.6/2 retitled and extended to the chord-change restart; AC-2.6.6/9, /10, /11 named tests over the pure deal), `tests/e2e/harmony.spec.js` (AC-2.6.6/1 retitled for the grouped catalogue; AC-2.6.6/12 named test on the note bands).
- [X] T259 **[new capability]** Sheet music: the score model — `specs/001-rhythm-master-mvp/spec.md` (US-12.2 added, AC-12.2.1–AC-12.2.11; AC-15.1.8 clarified — the grid section holds the grid or the score), `specs/001-rhythm-master-mvp/plan.md` (P-042), `specs/001-rhythm-master-mvp/research.md` (D-012), `specs/001-rhythm-master-mvp/data-model.md` (§6 `patternView`), `src/storage/settings.js` (`patternView` default), `src/core/notation.js` (new: a Pattern and a counting system → a score — per pass and Measure, each Beat's Slots as notes and rests with written values, dots, tuplets, ties, beams, staff steps from the spelled note, accidentals against the key signature and the Measure so far, the key signature for a Key and scale, accent marks, counting labels with their sounding state, chord names where the chord in force changes, the head's tempo mark and meter changes). Implements AC-12.2.2/2–/5, AC-12.2.4, AC-12.2.5/1–/6, AC-12.2.6, AC-12.2.7/1–/3, AC-12.2.8. Asked for by the maintainer 2026-09-12: "show me a sheet music version of it with annotations for the beat counting … the notes have to be either on the lines or between the lines."
- [X] T260 **[new capability]** Sheet music: the view, the toggle and printing — `src/ui/score.js` (new: the score drawn as inline SVG — staves, clefs, key signatures, meters, noteheads, stems, flags, beams, tuplet numerals, ties, rests, ledger lines, accidentals, accent marks, chord names, pass labels and counting labels; lines broken at Measure boundaries to the width given, a lone over-wide Measure scaled to fit; the current note marked from the transport position; printing via the browser dialog with the document title set to the Pattern's name), `src/main.js` (a Grid | Sheet toggle and a Print / PDF control at the head of the grid section; the score rendered in the grid's place from the same Pattern and transport position), `src/styles/tokens.css` (score, toggle, and a print stylesheet that leaves only the score on the page), `tests/e2e/responsive.spec.js` (AC-15.1.8's expected order names the grid section by its new wrapper, per the clarified AC). Implements AC-12.2.1, AC-12.2.2/1, AC-12.2.3, AC-12.2.5/7, AC-12.2.7/4, AC-12.2.9, AC-12.2.10, AC-12.2.11. Extends P-042.
- [X] T261 **[test]** Tests for T259 — `tests/unit/core/notation.test.js` (new: AC-12.2.2/2–/5, AC-12.2.4/1–/9, AC-12.2.5/1–/7, AC-12.2.6, AC-12.2.7/1–/3, AC-12.2.8/1–/3 named tests over the pure score model, including every on/off combination of every Recipe totalling its Beat; every written note's sounding pitch equals the timeline's for the same Slot and pass), `tests/unit/ui/score.test.js` (new: the same criteria that describe what is drawn — AC-12.2.2/2–/5, AC-12.2.4/1–/9, AC-12.2.5/1, /3–/6, AC-12.2.6, AC-12.2.7/1–/3, AC-12.2.8/1–/3 — proved on the rendered markup: noteheads, rests, beams, dots, ties, tuplet numerals, accidentals, accent marks, labels and chord names, read back from the svg the view writes).
- [X] T262 **[test]** Tests for T260 — `tests/e2e/score.spec.js` (new: AC-12.2.1/1–/5, AC-12.2.2/1, AC-12.2.3, AC-12.2.5/2, AC-12.2.5/7, AC-12.2.7/4, AC-12.2.9/1–/2, AC-12.2.10/1–/3, AC-12.2.11/1–/5 named tests, DOM-proven: staff positions measured from the drawn noteheads against the drawn staff lines, print media emulated for what the page carries, the print call observed).
- [X] T263 **[new capability]** Cycle through the fill patterns while practising — `specs/001-rhythm-master-mvp/spec.md` (US-2.7 added, AC-2.7.1–AC-2.7.3; AC-2.6.6/8, AC-2.6.7/7, AC-2.6.10 and AC-12.2.2 clarified), `specs/001-rhythm-master-mvp/plan.md` (P-043), `specs/001-rhythm-master-mvp/research.md` (D-011 amended: the fill in force is a playback overlay), `specs/001-rhythm-master-mvp/data-model.md` (§6 `fillCycleRepeats`), `src/core/harmony.js` (`fillInForce`: the catalogue index for a loop count from the starting fill, the base loop, the repeats and the harmonic cycle; `withArpeggio`: the Pattern with a fill substituted), `src/main.js` (cycle state — on, base loop — and the derived Pattern handed to the transport, the grid, the strips, the harmony controls and the score; the boundary handled in the loop callback; Stop and the toggle resetting the base; `onFillCycle`, `onFillCycleRepeats`), `src/ui/controls.js` (the Cycle toggle and Repeats count in the harmony section beside the Arpeggio picker), `src/ui/score.js` and `src/core/notation.js` (the head's Fill line), `src/storage/settings.js` (`fillCycleRepeats` default), `src/styles/tokens.css`. Implements AC-2.7.1, AC-2.7.2, AC-2.7.3. Asked for by the maintainer 2026-09-12: "cycle through the fill patterns one by one … default it to four times, and then have it cycle to the next one."
- [X] T264 **[test]** Tests for T263 — `tests/unit/core/harmony.test.js` (AC-2.7.2/1, AC-2.7.2/3 named tests over `fillInForce` and `withArpeggio`), `tests/unit/export/export.test.js` (AC-2.7.3/2), `tests/unit/ui/score.test.js` (AC-2.7.3/1: the head's Fill line as drawn), `tests/e2e/harmony.spec.js` (AC-2.7.1/1–/5, AC-2.7.2/2, /4–/7 named tests, DOM-proven; the advance observed on the real transport at a fast tempo with the count at 1).
- [X] T265 **[lint]** One prefer-const error in the drawn-score tests — `tests/unit/ui/score.test.js` (a `let` that is never reassigned, at AC-12.2.4/8's test). Deploy run 86 failed at Lint after T259–T262 merged, leaving the site stale; caught by CI, not locally, because the last edit to the file landed after the last local lint run. No assertion changes.
- [X] T266 **[new capability]** Condense a Pattern to its simplest subdivision — `specs/001-rhythm-master-mvp/spec.md` (US-10.2 added, AC-10.2.1–AC-10.2.5), `specs/001-rhythm-master-mvp/plan.md` (P-044), `src/core/condense.js` (new: `canHalve`, `halve`, `canCondense`, `condense` — each pair of consecutive Beats packed into one Straight 16ths Beat and each pair of Measures into one, repeated until no halving is possible; the Time Signature kept), `src/ui/controls.js` (the Condense button beside Double Length, disabled when no halving is possible), `src/main.js` (`onCondense`: the same landing as Double Length — in place on an owned Pattern, through the naming prompt on a shipped one). Implements AC-10.2.1–AC-10.2.5. Asked for by the maintainer 2026-09-12: "a lowest common denominator type feature … four measures of quarter notes could be one measure with sixteenth notes … a button … that should detect if it's even possible."
- [X] T267 **[test]** Tests for T266 — `tests/unit/core/condense.test.js` (new: AC-10.2.1/1–/5 and AC-10.2.2 named tests over the pure core, plus the halvability rules of AC-10.2.3/2–/5 as `: the core rule` qualifiers), `tests/e2e/operations.spec.js` (AC-10.2.3/1–/5, AC-10.2.4, AC-10.2.5 named tests, DOM-proven: the control's enabled state, the auto-saved store, the naming prompt).
- [X] T268 **[spec revision + data]** A bigger chord-progression catalogue, grouped by style — `specs/001-rhythm-master-mvp/spec.md` (AC-2.6.1/1 revised: the picker files the catalogue under seven group headings; AC-2.6.1/8–/14 added, one per group naming its entries; AC-2.6.1/15 added: no two entries spell the same chords under the default scale), `specs/001-rhythm-master-mvp/plan.md` (P-041), `src/core/harmony.js` (`PROGRESSION_GROUPS`; the catalogue grown from 15 to 86 entries, each carrying its group — the repeat shapes of every three-chord progression as entries of their own, the four rotations of the pop loop, the Royal Road, Creep, Something, Wonderwall, Hotel California, the minor-key loops with their qualities explicit, the jazz turnarounds, backdoor, tritone substitution, Autumn Leaves and the Coltrane changes, five twelve-bar blues forms and the eight-bar, the modal vamps and the Mario cadence, the Passamezzo antico, Romanesca and Folia), `src/ui/controls.js` (the picker renders one `optgroup` per heading, None first outside them), `src/styles/tokens.css` (heading styling in the picker). Implements AC-2.6.1/1, AC-2.6.1/8–/15. Extends P-041. Asked for by the maintainer 2026-09-13: "I don't feel there's enough chord progressions in the list … one four five five … one one four five … one four four five … they should probably be categorized … in the drop down." Stored Patterns hold spelled chords, not catalogue ids, so nothing migrates; the picker's first-match naming under a minor scale is unchanged and noted in the AC.
- [X] T269 **[test]** Tests for T268 — `tests/e2e/harmony.spec.js` (AC-2.6.1/1 rewritten for the headings: None first, seven `optgroup`s in order, every entry under one; AC-2.6.1/8–/14 named tests, one per group, each reading the group's entries and labels back from the DOM in order and choosing a few to read their chord names off the strip), `tests/unit/core/harmony.test.js` (AC-2.6.1/15: every entry spells chords of its own under Ionian and is matched back to its id; every entry files under a real heading, ids are unique, none exceeds the chord cap).
- [X] T270 **[spec revision + data]** A melodic twin of every exercise, each on its own progression, chord qualities and fill — `specs/001-rhythm-master-mvp/spec.md` (AC-2.5.4/4 revised: every shipped Melodic Pattern carries its scale explicitly, a catalogue id, no longer necessarily Ionian), `data/seed-patterns.json` (97 Patterns appended, one per `Exercises` line, each the same rhythm, tempo and tags as its percussive original with `soundMode: "melodic"`, a Key, a scale, every sounding Slot at the Root and a `harmony` block — a progression, a change setting and a fill — so the Family view pairs each twin with its original). Implements AC-2.5.4/4 as revised. Asked for by the maintainer 2026-09-13: "keep the percussive version, but … create melodic versions of those patterns as well … specifically use the fills and chord progressions feature … don't use the same one for every single one … make sure you use the chord qualities features as well."

  **How each twin was chosen** (a generator over `core/harmony.js`'s own catalogue, run once; the file is the record): the level tag picks the progression pool — `Beginner` from *Three chords and repeats* and *Modal and rock* plus I7–IV7–V7; `Intermediate` from *Pop*, *Minor* and *Classical and folk* plus the eight-bar and two twelve-bar blues; `Advanced` from *Jazz* plus the remaining four blues forms — and every one of the 86 entries is used, none more than twice, never twice with the same fill. Within a pool the entry whose chord count fits the exercise's bar count is preferred, and the change setting follows: `measure` when the counts divide (a four-chord loop over four bars is one chord a bar; a twelve-bar form over four bars resolves in three passes), `pass` otherwise. The scale matches the progression — Ionian for the major groups, Aeolian or harmonic minor for the minor ones, Dorian, Lydian, Phrygian and Mixolydian for the modal vamps, the two blues scales for the blues — and the fill pool follows the level too: chord-tone shapes for `Beginner`, drones, octave shapes and major walks for `Intermediate`, minor and pentatonic walks for `Advanced`; all 23 fills are used. Chord qualities are dealt from a per-level palette over the spelled chord — sus2, sus4, add9 and 6 on the Beginner lines (the tonic left plain), sevenths, 9ths, m6 and mMaj7 on Intermediate, 9ths, aug, dim7 and m7♭5 on Advanced — so all 19 qualities appear. Keys go round the circle of fifths. Each name is the original's plus the numerals it actually carries and the fill, e.g. *Quarter Notes — Filling In · I–IV6–V–I · Up and down*; a form longer than six chords is named for the form (*Twelve-bar blues*, *Autumn Leaves*, *Coltrane changes*). Because the qualities are coloured, the progression picker reads None on most twins — they are the Composer's own chords, as AC-2.6.1 says of any edited progression.
- [X] T271 **[test]** Tests for T270 — `tests/e2e/melodic.spec.js` (AC-2.5.4/4 retitled to the revised wording and its assertion changed to match: every shipped Melodic Pattern's scale is an id from `src/core/scales.js`'s catalogue, not necessarily `ionian`), `tests/unit/storage/seed.test.js` and `tools/validate-seed.js` unchanged and now exercising chord-tone Pitches and `harmony` blocks on shipped data for the first time.
- [X] T272 **[spec revision + layout]** Regroup the main panel by what the musician is doing — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.7 rewritten with Cases /1–/5: the workbench is tabbed on mobile and stacked above it; AC-15.1.8 rewritten for the new order; AC-15.1.17 added: Play/Stop and the exact tempo entry ride in the pinned bar; AC-2.1.6 added: the Sound Mode switch is in the header; AC-1.3.12, AC-2.2.13, AC-2.2.19/2, AC-4.2.5, AC-4.2.6, AC-4.4.14–AC-4.4.16, AC-2.7.1 and /1, AC-5.5.3 and AC-15.1.16 reworded for where each control now lives), `specs/001-rhythm-master-mvp/plan.md` (P-005, P-007, P-011, P-013, P-014, P-016, P-027, P-036, P-043), `src/ui/controls.js` (the header's Percussive | Melodic switch; `renderPlayControls` is Play/Stop plus the tempo entry; `renderMelodyGroup` — pitch strip then harmony; `renderRhythmGroup` — Subdivision chips then −Measure, + Measure, Double Length, Condense, Append…; `renderPracticeGroup` — Click, Count-in, tempo slider and presets, swing, counting, fill cycling; `renderBrushHint` under the grid; Pattern actions with Delete last; the Playback settings, Edit and Sound renderers retired), `src/main.js` (the pinned bar carries the transport; the workbench tab bar and `workbenchTab` state, reset on load and Mode change; `onWorkbenchTab`; the three groups and the actions accordion in the new order), `src/ui/responsive.js` (`applyAccordions` keeps only Pattern actions; `applyWorkbench` resolves the tab in force), `src/styles/tokens.css` (the tab bar, group titles, the pinned transport, Delete set apart). Implements AC-2.1.6/1–/3, AC-15.1.7/1–/5, AC-15.1.17/1–/4; revises AC-15.1.8, AC-1.3.12, AC-2.2.13, AC-2.2.19/2, AC-4.2.5, AC-4.2.6, AC-2.7.1/1, AC-5.5.3, AC-15.1.16. Extends P-036 and the plan items above. Asked for by the maintainer 2026-09-13 after the usability review of the same day ("there's a lot of menus, and I finally have to keep clicking around the menus and looking for things … is the tool organized in the best way"), which walked six tasks and found the panel organised by kind of control rather than by job, the Mode buried, and Play below the fold of a phone.
- [X] T273 **[test]** Tests for T272 — `tests/e2e/responsive.spec.js` (AC-15.1.7/1–/5 and AC-15.1.8 rewritten for the tabbed workbench and the new order; AC-15.1.16/1, /3 and /5 driven through the header switch and the Practice tab), `tests/e2e/library.spec.js` (AC-15.1.17/1–/4 named tests, DOM-proven with the panel scrolled to its foot), `tests/e2e/melodic.spec.js` (AC-2.1.6/1–/3 named tests; AC-2.2.13 retitled verbatim; AC-2.2.19/2 retitled; every Sound select call becomes a tap on the switch), `tests/e2e/harmony.spec.js` (AC-2.7.1/1 retitled and pointed at the Practice group), `tests/e2e/grid.spec.js` (the brush line found under the grid; AC-15.2.8/4 measured against the Practice group's first control), `tests/e2e/operations.spec.js`, `tests/e2e/remaining.spec.js`, `tests/e2e/duplicates.spec.js` (selectors for the moved controls).
- [X] T274 **[bug]** The −Measure control, specified in the MVP and never built — `src/ui/controls.js` (−Measure beside + Measure in the Rhythm group, disabled on a one-Measure Pattern), `src/main.js` (`onRemoveMeasure`: the last Measure, through the shipped-Pattern naming flow like every other edit), `tools/unwired-baseline.json` (the `removeMeasure` entry pruned — the finding the gate was built for), `specs/001-rhythm-master-mvp/spec.md` (AC-1.1.8 gains Cases /1–/2), `specs/traceability-baseline.json` (the AC-1.1.8 and AC-1.1.9 findings pruned). Implements AC-1.1.8/1–/2, AC-1.1.9. Extends P-005. Found by the 2026-09-13 usability review: the only ways to shorten a Pattern were Condense and Undo.
- [X] T275 **[test]** Tests for T274 — `tests/e2e/grid.spec.js` (AC-1.1.8/1, AC-1.1.8/2, AC-1.1.9 named tests, DOM-proven: the disabled state, Delete still offered elsewhere, the last Measure removed and the others untouched), `tests/unit/core/pattern.test.js` (the core rule retitled under AC-1.1.8/1 and AC-1.1.9 with a qualifier; the purity test no longer wears AC-1.1.9's name).
- [X] T276 **[spec revision]** Retire the chord-tone picker; a progression brings its own fill — `specs/001-rhythm-master-mvp/spec.md` (AC-2.6.5 rewritten: /1–/6 retired, old /7 kept as the new /2; AC-2.6.1/7 rewritten — choosing a progression sets the Ascending arpeggio and rewrites no stored Pitch, replacing the degree-to-role conversion; AC-2.6.6/7 reworded, the role chips dropped from it; AC-15.1.8's US-2.6 parenthetical reworded), `src/ui/controls.js` (the `.tone-group` role chips and `spellTone` removed from the pitch strip; the harmony hint now points at the Arpeggio setting alone), `src/main.js` (`onArmTone` retired; `armedFor` no longer translates a role; `onProgression` sets `DEFAULT_ARPEGGIO` and drops the armed-pitch re-read), `src/core/harmony.js` (`DEFAULT_ARPEGGIO`; `setProgression` leaves every stored Pitch untouched; `degreeAsTone` retired with the conversion it existed for). Implements AC-2.6.5/1, AC-2.6.5/2; revises AC-2.6.1/7, AC-2.6.6/7. Extends P-041. Asked for by the maintainer 2026-09-13, practising with it daily: "I don't see what kind of task would require the chord tones control … I am either going to just stamp melody notes using the original picker or choose a chord progression, specify chord qualities and fill." The control was also read as a display of the chords' tones and failed as one — it spells the chord in force, which at rest is always the first chord. The one task it served, a hand-written line whose shape transposes with the harmony, is a fill the catalogue does not hold; a custom-fill feature was offered and declined for now. The fill default is the maintainer's own answer to what the retired conversion was for: "why can't, when you pick a progression, you just automatically populate a fill anyway, as a default?" It is set every time, and the cost was named and accepted — a hand-written melody goes quiet under it, and AC-2.6.1/6's bake then writes the dealt line over it, which the maintainer chose to keep.
- [X] T277 **[test]** Tests for T276 — `tests/e2e/harmony.spec.js` (AC-2.6.5/1 and AC-2.6.5/2 named tests replacing the eight role-chip tests; an AC-2.6.1/7 named test proving the arpeggio is set, the melody is not rewritten and it comes back with the arpeggio off; `harmonicBlank` puts the arpeggio back to None so the stamping tests still stamp; AC-2.6.7/5's chord-tone Slot loaded rather than armed; AC-2.6.6/7 and AC-2.6.8/3 stamped with degrees), `tests/unit/core/harmony.test.js` (the three AC-2.6.1/7 tests replaced by one named for the new criterion — no stored Pitch changes, and a role stamped afterwards still resolves; the AC-2.6.6/7 name follows the reworded AC; the `degreeAsTone` import dropped), `tests/unit/core/notation.test.js` and `tests/unit/ui/score.test.js` (AC-12.2.5/5's fixtures stamp their chord-tone Pitch directly — a test defect under §2a: they had relied on the retired conversion to make one, and the criterion they prove is unchanged).
- [X] T278 **[test defect]** AC-15.1.17/1 failed four runs in five, on `main` as well as on this branch — `tests/e2e/library.spec.js` (`onScreen` polls its measurement). Not a change to what the criterion asserts: playback re-renders the transport on every tick, so the Stop button resolved a moment earlier is often detached by the time it is measured, and a detached node reports a 0×0 box at the origin — read by the old helper as "off screen". The box is still required to sit wholly within the viewport; only a measurement taken mid-render is retried. Found while running the gates for T276.
- [X] T279 **[spec revision]** A personalized Pattern takes nothing from the carry — `specs/001-rhythm-master-mvp/spec.md` (AC-4.2.3 gains Case /6 and its And-clause; AC-4.4.17 gains Case /6 and its And-clause), `specs/001-rhythm-master-mvp/plan.md` (P-011, P-013), `src/core/playback-defaults.js` (`carriedPlaybackFor` returns nothing for a Pattern with any overlay playback value — tempo, swing amount, feel, or a legacy per-group entry). Implements AC-4.2.3/6, AC-4.4.17/6. Extends P-011 and P-013. Asked for by the maintainer 2026-09-13: the carry decided field by field, so a Pattern they had set a tempo on still took the swing in effect on the Pattern just left — "if I've already gone to a pattern and made changes to it or adjusted the tempo … then I shouldn't automatically apply swing … nor should I override the tempo settings I already applied." The maintainer chose, when asked: "personalized" means touched. A Pattern opened and never adjusted still takes the carry, since opening writes nothing (AC-4.4.17) and a seen-flag would be new bookkeeping for no musical gain.
- [X] T280 **[test]** Tests for T279 — `tests/unit/core/playback-defaults.test.js` (AC-4.2.3/6 and AC-4.4.17/6 named tests with the `: the resolver` qualifier, covering a remembered amount, a remembered feel and a legacy per-group entry each blocking the carry on their own), `tests/e2e/playback.spec.js` (AC-4.2.3/6 and AC-4.4.17/6 driving the real open path: swing set on a shipped Pattern keeps a later tempo from carrying onto it, and a tempo set keeps a later swing and feel out).
- [X] T281 **[test defect]** AC-2.2.15/3 could not finish inside its 30s budget on a slow machine, on `main` as well as on the branch — `tests/e2e/melodic.spec.js` (the 8-Measure, 192-Slot 12/8 Pattern is built as data and loaded once). Not a change to what the criterion asserts: the same Pattern, Key, viewport and measurements, and the count, one-line and thin-strip expectations are untouched. The old setup tapped every Slot and stamped every pitch through the handlers — 216 awaited full re-renders before a single measurement — which T171 had already brought from 15.7s to 6.0s on CI and which still ran past 30s three times in three on `main` in the container the gates were run in for T279. Found while running the gates for T279.
- [X] T282 **[new capability + spec revision]** Keep the fills worth composing with; cycle mode returns to the Melody group — `specs/001-rhythm-master-mvp/spec.md` (US-2.8 added: AC-2.8.1/1–/6, AC-2.8.2/1–/2; AC-2.7.1 and /1 revised; AC-15.1.7 and /4 revised; AC-15.1.8 revised; Key Entities gain Keep), `specs/001-rhythm-master-mvp/plan.md` (P-045; P-043, P-036), `specs/001-rhythm-master-mvp/data-model.md` (`rm.overlays.v1` gains `keptFills`), `src/storage/overlays.js` (`keepFill`, `unkeepFill`, `keptFillsFor`), `src/ui/controls.js` (cycle controls and Keep move into `renderMelodyGroup`; the Arpeggio picker marks kept fills; a kept count), `src/main.js` (`onKeepFill`; the Melody group renders the cycle state). Implements AC-2.8.1/1–/6, AC-2.8.2/1–/2; revises AC-2.7.1/1, AC-15.1.7/4, AC-15.1.8. Asked for by the maintainer 2026-09-13 (see the US-2.8 note in spec.md).
- [X] T283 **[test]** Tests for T282 — `tests/unit/storage/storage.test.js` (keeps are per Pattern, survive a reload, and are barred from exports), `tests/e2e/harmony.spec.js` (AC-2.8.1/1–/6, AC-2.8.2/1–/2 named tests driving Keep while cycling; AC-2.7.1/1 retitled to the Melody group), `tests/e2e/responsive.spec.js` (AC-15.1.7/4 and AC-15.1.8 retitled and extended for the fourth group).
- [X] T284 **[new capability]** Compose a Section from kept fills — `specs/001-rhythm-master-mvp/spec.md` (US-18.1: AC-18.1.1–AC-18.1.5; Assumptions' out-of-scope line narrowed; Key Entities gain Song and Section), `specs/001-rhythm-master-mvp/plan.md` (P-046), `specs/001-rhythm-master-mvp/research.md` (a D-0xx entry: the Section plays through the pass-boundary seam cycle mode already uses; a Song is its own store, keyed to Patterns by id), `specs/001-rhythm-master-mvp/data-model.md` (`rm.songs.v1`: Song `{id, name, sections: [{patternId, entries: [{fill, repeats}]}]}`; validation rules; Pattern deletion refused while referenced), `src/core/song.js` (new, pure: entry at pass, total passes, validation), `src/storage/songs.js` (new), `src/ui/compose.js` (new: `renderComposeGroup`), `src/main.js` (Play song through the `onLoop` seam; Save, rename, delete; the fourth tab), `src/ui/responsive.js` (`workbenchTabFor` knows Compose), `src/main.js` (delete refused naming the Song), `src/export/midi.js` (`buildSongMidi`, `downloadSongMidi`), `src/styles/tokens.css`. Implements AC-18.1.1/1–/3, AC-18.1.2/1–/5, AC-18.1.3/1–/7, AC-18.1.4/1–/7, AC-18.1.5/1–/4. Asked for by the maintainer 2026-09-13 (see the US-18.1 note in spec.md).
- [X] T285 **[test]** Tests for T284 — `tests/unit/core/song.test.js` (new: entry-at-pass and total-pass arithmetic, named for AC-18.1.3/1 and AC-18.1.2/4 with a `: the resolver` qualifier), `tests/unit/storage/storage.test.js` (`rm.songs.v1` round-trip; a referenced Pattern cannot be removed), `tests/unit/export/song-midi.test.js` (AC-18.1.5/1–/3: the Section once through, the Pattern export unchanged), `tests/e2e/compose.spec.js` (new: AC-18.1.1/1–/3, AC-18.1.2/1–/5, AC-18.1.3/1–/7, AC-18.1.4/1–/7, AC-18.1.5/2, /4 named tests driving the real Compose group), `tests/e2e/responsive.spec.js` (AC-15.1.7/1–/4 extended for the Compose tab).
- [X] T286 **[spec defect]** The autoscroll stands down for the whole playback run, not two seconds — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.16's autoscroll And-clause and Case /2 rewritten; a dated parenthetical), `src/ui/responsive.js` (`AUTOSCROLL_GRACE_MS` retired), `src/main.js` (the main-panel interaction sets a stand-down flag instead of stamping a time; the flag clears on the not-playing → playing transition, so every Start tracks again and pressing Play does not stand its own run down). Revises AC-15.1.16/2. Extends P-036. Reported by the maintainer 2026-09-13: "every time the pattern repeats, it jumps back to the top of the screen … I have to keep stopping the melody and making the changes." The defect is in the AC, not the code — two seconds was a guess at how long an adjustment takes, and tweaking tempo or melody while listening takes far longer, so the grace expired mid-experiment and AC-15.1.11's loop back to Measure 1 hauled the view to the top of a long Pattern on every pass. AC-15.1.11 is untouched: it describes hands-off playback, which still tracks. The maintainer chose stand-down-for-the-run over a forward-only scroll, a Follow toggle and a longer grace, and confirmed the symptom at every width.
- [X] T287 **[test]** Tests for T286 — `tests/e2e/responsive.spec.js` (AC-15.1.16/2 retitled to the revised Case verbatim and its second half rewritten: one nudge, then six seconds hands-off — three times the retired grace and more than one loop of the tall Pattern — with the slider asserted on screen throughout; the resume is then proven by stopping and starting, which is what now restores tracking). A test change following a revised AC, per §2a: the AC changed first, in the same change, and nothing asserted is relaxed — the hands-off half became a stronger assertion than the one it replaced.
- [X] T288 **[bug + spec addition]** A rebuild that would change nothing does nothing, so an open picker survives playback — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.16 gains Case /7 and a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-036), `src/ui/controls.js` (`rebuild` returns untouched when the fresh markup equals what is on screen; a kept-alive `SELECT`/`BUTTON`'s children are replaced only when they differ; `value`, `checked` and `disabled` are assigned only when they differ). Implements AC-15.1.16/7. Extends P-036. Reported by the maintainer 2026-09-13 from the TV: opening the chord-progression picker during playback left it "flickering on and off, and I can't actually select anything", so changing a progression meant stopping first. Playback renders on every sounding event and these panels do not change between ticks — the rebuilds were identical and did nothing, but each one replaced the `<select>` (or, under the keep-alive, its whole option list), and a native popup does not survive either. Measured before and after on the real page: the picker node was destroyed within the first tick, and now survives four seconds of playback both focused and unfocused. The maintainer's alternative — stopping playback automatically when a picker opens — was offered and declined, since it takes back the tweak-while-playing the same day's T286 delivered.
- [X] T289 **[test]** Tests for T288 — `tests/e2e/harmony.spec.js` (AC-15.1.16/7 named test: the picker and every one of its option nodes are held by reference and asserted still in the document across four seconds of playback focused, then again unfocused — the VIDAA case, where no keep-alive engages — and a progression chosen mid-playback still takes, so the skip is not a freeze). Held by reference rather than marked with an attribute on purpose: the skip compares the markup the render wants against the markup on screen, so a marker written in by the test provokes the very rebuild under test. Verified red against the unfixed code.
- [X] T290 **[approach change]** Two panes on a wide panel, tabs at every width, and Measures that flow — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.7 revised: tabbed at every width, Case /2 reversed; AC-15.1.8 revised: the tab bar at every width, the list split across two panes on a wide panel; AC-15.1.12 clarified; AC-15.1.18 added with Cases /1–/5: the grid pane and the workbench pane side by side from 1024px of panel width, each scrolling on its own, keyed on the panel's width not the viewport's; AC-15.1.19 added with Cases /1–/4: Measures flow left to right and wrap by whole Measure; AC-15.2.8 revised with Case /1 reworded for a column per pane), `specs/001-rhythm-master-mvp/research.md` (D-006 amended: as many whole Measures per row as fit, never split), `specs/001-rhythm-master-mvp/plan.md` (P-036 carries AC-15.1.18–AC-15.1.19; P-036 and P-038 extended), `src/ui/responsive.js` (`WIDE_PANEL_MIN`, `panelLayout`, `applyPanelLayout`, `observePanelWidth`, `scrollContainerOf`), `src/main.js` (the main panel holds a pattern pane and a workbench pane; the load-time scroll reset and the AC-15.1.16/3 height compensation address whichever pane scrolls), `src/ui/controls.js` (the group titles go — the tab is the title at every width), `src/styles/tokens.css` (the wide layout as a grid of two scrolling panes under the pinned bar; the tab bar and one-group-at-a-time at every width; the grid a wrapping row of Measure boxes; the content-column cap applied per pane). Implements AC-15.1.18/1–/5, AC-15.1.19/1–/4; revises AC-15.1.7/2, AC-15.1.8, AC-15.2.8/1. Asked for by the maintainer 2026-09-13 after the T272 regrouping: "in the TV and the tablet view, now it looks like there's a lot more scrolling required … I wanna keep everything into one screen wherever possible and use things like tabs." Measured before the change: an eight-bar Melodic Pattern with a progression was 2562px of panel on a 1920×1080 TV and 2602px on a 1180×820 tablet. The maintainer chose the two-pane layout over tabs-only and grid-top/controls-bottom, and set the Measure-flow rule ("don't split a measure across two rows"), 2026-09-13.
- [X] T291 **[test]** Tests for T290 — `tests/e2e/responsive.spec.js` (AC-15.1.7/2 retitled to the reversed Case and rewritten: the tab bar visible and one group on screen at desktop and tablet; AC-15.1.8's DOM walk reads the sections of both panes in order, since the criterion's order is unchanged and the flat-children walk was the test's own assumption; AC-15.1.16/3 and /5 rewritten to grow the grid with + Measure above a focused or held slider in the Practice tab — the old scenario, an inserted group moving the stacked group below it, no longer exists once the groups are tabbed at every width, and the criterion asks only that something above changes height; new tests for AC-15.1.18/1–/5 and AC-15.1.19/1–/4), `tests/e2e/grid.spec.js` (AC-15.2.8/1 retitled to the reworded Case and checked per pane; AC-15.2.8/2 drops the two-boxes-share-a-left-edge assertion the criterion never made and AC-15.1.19 contradicts, keeping every assertion the Case does make). Test changes following revised ACs, per §2a: the ACs changed first, in the same change.
- [X] T292 **[test]** AC-15.1.14/4 polls for the re-balance after the library toggle — `tests/e2e/grid.spec.js`. The deploy run for PR #83 (run 99) failed on this test alone: it read the Beat layout in the same turn as the toggle click, before the ResizeObserver that re-balances the grid had run, and saw the two-pane wrap instead of the one line. Same fix AC-15.1.14/5 already carried; nothing asserted is relaxed — the same layout is awaited rather than read at once.
- [X] T293 **[bug + spec addition]** A half-typed entry survives the renders playback streams — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.16 gains Case /8 and a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-036), `src/ui/controls.js` (a document-level `input` listener marks a number or text entry as holding uncommitted typing, and `change` or `focusout` clears it; the keep-alive's value write-back stands down for an entry so marked). Implements AC-15.1.16/8. Extends P-036. Reported by the maintainer 2026-09-13: typing 2 into Cycle fills' Repeats box on a playing Melodic Pattern left the cycle still moving on every four harmonic cycles. The cycle arithmetic was never at fault — `fillIndexFor` and `onFillCycleRepeats` are correct, and the same typing works with the transport stopped. Playback renders on every sounding event, and the keep-alive of AC-15.1.16/1, which preserves the entry's node, wrote the stored value back over the digits before the `change` event that commits them could fire; AC-15.1.16/7's skip cannot help, since a half-typed entry is exactly a rebuild that differs. Measured on the real page: `2` typed while playing read back as `4` within a fraction of a second, and the Tempo entry had the same fault — `120` typed during playback reverted to `300`. The write-back is kept for every entry not being typed into, which is what AC-7.1.1's cleared-name revert relies on.
- [X] T294 **[test]** Tests for T293 — `tests/e2e/responsive.spec.js` (two AC-15.1.16/8 named tests with qualifiers, driving real keystrokes against a playing Melodic Pattern at 300 BPM with every Slot of Beat 1 sounding: the Repeats count typed, still standing several renders later, and committed to the setting and to `rm.settings.v1` on leaving the entry; the Tempo entry typed a digit at a time, then a refused value left in it and the state's own value written back once committed — the AC-7.1.1 behaviour the fix deliberately keeps). Verified red against the unfixed code, both tests failing on the typed value reverting.
- [X] T295 **[new capability + spec addition]** The Register: place a whole Melodic Pattern in an octave — `specs/001-rhythm-master-mvp/spec.md` (US-2.9 added as User Story 44 with AC-2.9.1/1–/4, AC-2.9.2/1–/5, AC-2.9.3/1–/2, AC-2.9.4/1–/2; AC-2.2.19 gains Case /4 and a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-047 added; P-016 extended), `specs/001-rhythm-master-mvp/data-model.md` (§1 `register` field, §4 resolution now `(degree, octaveOffset, key)` plus the Pattern's register, §4 computed-value table, §7 rule 19), `src/core/pitch.js` (`REGISTERS`, `MIN_REGISTER`/`MAX_REGISTER`, `registerOf`, `soundingOctaveOffset`, `inRegister` — the one conversion from a stored Pitch to a sounding one), `src/core/timeline.js` (the Register applied once, where playback, the cursor and the MIDI file share a timeline), `src/core/notation.js` (the score writes the sounding octave), `src/core/pattern.js` (validation), `src/ui/grid.js` (the note band names what sounds), `src/ui/controls.js` (the Register picker on the pitch strip; the Key picker gains its own visible label), `src/main.js` (`onRegister`, guarded on a shipped Pattern as the Key is; Percussive strips the field), `src/export/submit.js` (a submission carries the Register), `tools/validate-seed.js` (rule 19). Implements AC-2.9.1–AC-2.9.4, AC-2.2.19/4. Adds P-047; extends P-016. Reported by the maintainer 2026-09-13: "I would think the key and the octave should apply to the corresponding chord progression that I pick … I'm still hearing the same progression no matter what. And this is where I kinda want to do things like write a bass line versus writing like a keyboard fill." Diagnosis: the Key already applies to everything, because it resolves at play time; the octave stepper arms the next stamp (AC-2.2.3) and so had nothing to move under an arpeggio dealing every note. The Register is the missing control — Pattern content, applied at resolution, rewriting no stored Pitch. The maintainer chose it over making the stepper transpose the stamped line, and approved the spill (the score, the note bands and the MIDI file all move with it) before the work.
- [X] T296 **[test]** Tests for T295 — `tests/unit/core/timeline.test.js` (AC-2.9.2/1–/4 named tests over the pure timeline: the octave shifts at all four steps, dealt arpeggio steps and chord-tone Pitches moving as a stamped degree does, no stored Pitch rewritten, and the 1–7 clamp at both ends), `tests/unit/export/export.test.js` (AC-2.9.2/5: the MIDI note-ons at a Register, equal to the timeline's), `tests/unit/core/notation.test.js` (AC-2.9.3/2, qualified: the written note, its staff step and its head at the sounding octave in the score model), `tests/e2e/score.spec.js` (AC-2.9.3/2 on the drawn staff: the notehead falls seven staff steps at Bass), `tests/e2e/melodic.spec.js` (AC-2.9.1/1–/4 on the strip, the reload, the shipped-Pattern naming guard and Percussive; AC-2.9.3/1 on the note band; AC-2.9.4/1 and /2 separating the stepper from the Register; AC-2.2.19/4 on the two labels). Verified red against the unfixed code: with the Register not applied in `buildTimeline`, all four AC-2.9.2 tests and the AC-2.9.4/2 e2e fail.
- [X] T297 **[spec defect + bug]** The autoscroll stands down on the view moving, not on an input event — `specs/001-rhythm-master-mvp/spec.md` (AC-15.1.16/2 widened to name scrolling; AC-15.1.16/9 added; a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-036), `src/main.js` (`wheel` and `touchstart` join the captured input listeners; a captured `scroll` listener on `window` stands the autoscroll down for any scroll of the panel, its panes or the document that the app did not itself perform; `appScrolled()` marks the app's own scrolls — the autoscroll, the height compensation of AC-15.1.16/3, the reset on loading a Pattern — and the autoscroll marks only the renders on which it actually scrolled). Implements AC-15.1.16/9; revises AC-15.1.16/2. Extends P-036. Reported by the maintainer 2026-09-13 from an iPhone added to the Home Screen, the third report of this symptom: "I scroll down to try to make some tweaks, but then it keeps scrolling right back up to the pattern." T286 implemented the stand-down as `pointerdown`, `input` and `keydown` on the panel, and a scroll is none of those; a finger drag raises `pointerdown` before it scrolls, which is why it passed in a browser and on the devices it was checked against, but momentum carrying on after the finger lifts raises nothing. Reproduced before the fix: the panel scrolled to 1407px during playback was back at 241px within 700ms; after it, 1407px holds through four seconds of playback and AC-15.1.11's hands-off tracking still follows the sounding Measure (measured 0 → 18 → 44 → 249). The first attempt marked every playback render as an app scroll, which held the exclusion window permanently open and changed nothing — caught by running the reproduction rather than the suite.
- [X] T298 **[test]** Tests for T297 — `tests/e2e/responsive.spec.js` (AC-15.1.16/9 named test: during playback the panel is scrolled with no input event of any kind and the offset is asserted unchanged across four seconds; AC-15.1.16/2's test retitled to the widened Case verbatim). Verified red against the unfixed code — the offset was gone within 500ms. Two test defects were found and fixed while proving it, both by running the test against the unfixed code rather than by reading it: it first failed under parallel load because it set `scrollTop` while a smooth autoscroll was still animating, which an assignment cannot cancel though a real gesture can (it now waits for the panel to come to rest first); and the wait, as first written, re-read the offset *after* the yank, which made the test pass against the unfixed code by asserting only that the view stayed yanked (the offset is now set and read in one evaluation).
- [X] T299 **[bug]** The playback highlight keeps pace with a clamped poll timer — `src/audio/scheduler.js` (`flushVisuals` also runs from a `requestAnimationFrame` loop started in `start()` and cancelled in `stop()`; the loop is a flush only — it schedules no audio and never moves the origin, and a browser with no `requestAnimationFrame` keeps the tick-driven flush unchanged). Implements AC-4.1.2. Extends P-010. Reported by the maintainer 2026-09-13 from a Samsung TV's in-app browser: "the sound is ahead of the actual notes being lit up on the screen", more than a beat out at 80 BPM. T242 fixed the audio half of exactly this throttling on the Hisense/VIDAA browser by widening the lookahead, and that is why the audio sounds dead on the grid here — events are handed to Web Audio a clamp ahead of when they sound. The highlight was left flushed from inside that same clamped tick, so it could only advance as often as the timer fired: more than 750ms behind at 80 BPM is a timer clamped to about a second, not panel display lag, which is why no Game Mode setting would have helped. An animation frame is driven by the compositor rather than the timer and keeps arriving while `setInterval` is clamped; the queue is still read against `ctx.currentTime`, so the frame changes only how often the queue is looked at, never what time an event carries.
- [X] T300 **[test]** Tests for T299 — `tests/unit/audio/scheduler.test.js` (three AC-4.1.2 named tests with qualifiers, driving the transport with a fake audio clock, a stubbed frame queue and the poll timer firing exactly once in the window: every highlight within the criterion's 20ms of its own sounding time, no highlight arriving before the event it marks, and the tick-driven flush still delivering with no `requestAnimationFrame` present at all). Extends P-010. Verified red against the unfixed code: 13 events sounded on the grid and only the 2 the single tick happened to flush were highlighted.
- [X] T301 **[bug]** Playback no longer rebuilds the whole application on every sounding event — `src/storage/keyValue.js` (`readStore` memoizes the parsed value against the raw string it came from; `rawOf` exposes that raw string as a cheap change key), `src/storage/patterns.js`, `src/storage/overlays.js`, `src/storage/localMeta.js`, `src/storage/songs.js` (the five write paths that mutated what a read handed back now copy first, which is what makes the read cache safe), `src/ui/grid.js` (`moveCursor` — the cursor moves by swapping one class instead of rebuilding every Measure, Beat and Slot), `src/main.js` (`render(kind)` with a `cursor` path taken by `onPosition`, redrawing only what depends on `transportPosition`; the library list and the Family list memoized on the raw stores plus the view state; the autoscroll extracted to `followCursor`, shared by both paths). Implements AC-4.1.2. Extends P-010. Reported by the maintainer 2026-09-13 after T299 failed to fix it: "even if I just click something in the library, like a selector for the filters … it's probably two seconds before it actually takes effect", and the highlight still behind the audio. T299's diagnosis was wrong — a filter click has no audio and no timer in it, so the cause was never the clamped poll. Profiled at 20x CPU throttle: four seconds of playback blocked the main thread for 11.3 seconds, longest single task 7.0s. Two causes, both measured. `libraryEntries()` called `overlays.applyTo` and `overlays.addedTagsFor` once per Pattern and each re-read and re-parsed the entire overlay store, so building the list re-parsed localStorage roughly six hundred times; and that list, with the header, five control groups, the workbench and the Family list, was rebuilt on every sounding event. After: 822ms of blocking over the same window, longest task 292ms — playback 14x better, and the tag-filter click 534ms to ~350ms. The click is improved rather than fixed: the residue divides about evenly between the 305 library rows and the other control groups, and closing it means memoizing each group separately, which is a further change and a further risk.
- [X] T302 **[test]** Tests for T301 — `tests/e2e/playback.spec.js` (two AC-4.1.2 named tests with qualifiers: the fast path's DOM compared byte for byte against the pure render's at five positions including the cleared cursor, so the two implementations of AC-4.1.2's rendering cannot diverge; and a real playing Pattern where the cursor visits several Slots while the grid container is rebuilt at most at pass boundaries). Extends P-010. Verified red against the unfixed code — with `onPosition` restored to a full render the grid was rebuilt on every sounding event and the rebuild count assertion failed. The first draft of that test asserted the wrong thing: it counted class-toggle mutations, which a full render never raises because it replaces the nodes instead, so it failed for the wrong reason. Caught by reading the red output rather than trusting the red.
- [X] T303 **[new capability]** Indie folk/rock and suspended progressions in the catalogue — `specs/001-rhythm-master-mvp/spec.md` (AC-2.6.1/1 revised to nine group headings; AC-2.6.1/16 and AC-2.6.1/17 added), `specs/001-rhythm-master-mvp/plan.md` (P-041), `src/core/harmony.js` (`PROGRESSION_GROUPS` gains Indie and alt and Sus and open chords; 28 entries added — 13 indie folk/rock loops and 15 suspended, add9 and m7 entries — taking the catalogue from 86 to 114). Implements AC-2.6.1/16 and AC-2.6.1/17, revises AC-2.6.1/1. Asked for by the maintainer 2026-09-14: "I'm also looking for some of the indie folk, like, indie rock type of progressions … I also like a lot of bands that use sus2, sus4 … I'm not sure how to adapt the existing ones … include other chords that are common with sus progressions as well." Duplicates were ruled out by AC-2.6.1/15's existing gate, which is what decided the candidates that were dropped.
- [X] T304 **[test]** Tests for T303 — `tests/e2e/harmony.spec.js` (AC-2.6.1/1's test renamed and its heading list and catalogue-size floor raised; AC-2.6.1/16 and AC-2.6.1/17 named tests added, DOM-proven: each group's entries in order, and the chords each names in C).
- [X] T305 **[spec defect + bug]** The octave stepper stands down once the Register can move the same note — `specs/001-rhythm-master-mvp/spec.md` (AC-2.9.5 added with Cases /1–/3 and a dated parenthetical, revised the same day), `specs/001-rhythm-master-mvp/plan.md` (P-047 extended), `src/ui/controls.js` (`renderPitchStripInto`: the Register picker is unconditional, as before; the octave stepper, at both its call sites, renders only when `!hasHarmony(pattern)`). Implements AC-2.9.5/1–/3. Extends P-047. Reported by the maintainer 2026-09-13, painting individual notes: "the Octave mode doesn't really make any sense... when I'm using a chord progression. I have to use the Register. But when I'm painting individual notes, then I should use the Octaves." Since US-2.9 landed the same day, the Register and the octave stepper had sat side by side on every Melodic Pattern's pitch strip, the octave stepper inert once a progression exists (AC-2.9.2 routes every sounding note through the Register instead). First implemented as hiding the Register too on a Pattern with no progression, which broke six of US-2.9's own tests and contradicted AC-2.9.2, which lets the Register move a plain melody with no progression at all. Asked to reconcile, the maintainer confirmed the two controls are not redundant even without a progression — the octave stepper decides what octave each new stamp gets while composing, the Register then shifts the whole finished shape as one block — so only the octave stepper is mode-exclusive; the Register stays on every Melodic Pattern. Mode is read from `hasHarmony`, chosen over gating on the arpeggio alone because a progression with the arpeggio set to None still resolves through the Register; the maintainer confirmed, when asked, that a fixed note hand-stamped in that state should land at octave-offset 0 rather than have the stepper reappear for that one sub-case.
- [X] T306 **[test]** Tests for T305 — `tests/e2e/melodic.spec.js` (AC-2.9.5/1 named test: a Pattern with a progression and an arpeggio, and separately one with the arpeggio set to None, both show the Register picker and no octave stepper on the pitch strip; AC-2.9.5/2 named test: a Pattern with no progression shows both the Register picker and the octave stepper; AC-2.9.5/3 named test: adding a Pattern's first chord hides the octave stepper on that render, leaving the Register in place, and removing its last chord brings the stepper back, with no reload). Verified red against the unfixed code.
- [X] T307 **[test defect]** AC-2.7.1/5 reads `pendingEdit` in the turn that sets it — `tests/e2e/harmony.spec.js`. The test was failing on `main`, deterministically in three full runs and before any of T305's work: it clicked the Cycle toggle while playing, then read `window.__rm.transport._snapshot().pendingEdit` through a *second* `page.evaluate` and expected it still set. The code is right and stays untouched. `pendingEdit` is transient by design — the scheduler clears it the moment it crosses the next pass boundary and swaps the edited Pattern in (`src/audio/scheduler.js`, the `if (pending)` branch) — and `cyclingBlank` runs at 300 BPM, a 0.8s pass against a ~0.42s lookahead, so a boundary routinely lands inside the round trip. Measured: sampled from inside the page the flag reads `true` on the first sample and `false` 20ms later; driven by six separate Playwright clicks it read `true` four times and `false` twice, which is the race. Fixed per §2a as a test defect, not a code one: **AC-2.7.1/5 settles it** — it asserts the Pattern's own arpeggio returns "from the next pass while playing", which is what the test's surviving assertions check (the run keeps going, and the snapshot's arpeggio is back to null a pass later); it never asserts that an internal `pendingEdit` seam is observable at an arbitrary instant afterwards, which is the assumption the test had encoded. Nothing is relaxed — the same claim, that the edit was deferred rather than applied at once, is now read in the same synchronous turn as the click, where no scheduler tick can interleave.
- [X] T308 **[spec defect]** Best-effort background playback — `specs/001-rhythm-master-mvp/spec.md` (AC-4.1.5 revised: a recoverable suspension pauses rather than stops-and-resets; AC-4.1.6 revised with Cases /1–/2 and a dated parenthetical; AC-4.1.11 added; AC-2.7.2/6 and AC-4.1.10 gain cross-reference parentheticals), `specs/001-rhythm-master-mvp/plan.md` (P-010's AC range extended to AC-4.1.11), `src/audio/context.js` (`onResumed` added alongside `onSuspended`; the `statechange` listener fires it on a return to `'running'`, and `visibilitychange` fires it on a return to `'visible'`, mirroring the suspend-side detection each already had), `src/audio/scheduler.js` (`createTransport` gains `onSuspend`/`onResume` callbacks; `_handleSuspend` pauses scheduling and sounding without resetting the pass or Slot position; `_handleResume` calls `resume()` and, only when the context comes back running **on the same context object that was suspended**, re-anchors the origin exactly as a throttled timer's stall does (`recoverFromStall`) and continues the run — a context that stays stuck, or that `resume()` replaces with a different object, falls back to `stop()`, unchanged), `src/audio/mediaSession.js` (new: `setPlaying`/`setPaused`/`setStopped`, entirely feature-detected on `navigator.mediaSession`), `src/main.js` (the transport's `onSuspend`/`onResume` wired to `mediaSession`; `onPlay` and the transport's `onStop` call it too). Implements AC-4.1.11/1–/2, revises AC-4.1.5, AC-4.1.6 (adds Cases /1–/2), AC-2.7.2/6 and AC-4.1.10 (cross-reference only, no behaviour change). Extends P-010. Asked for by the maintainer: practising hands-free with the phone screen off, the device backgrounding the tab or locking the screen inside a few minutes of a normal practice run was resetting playback to the top of the Pattern every time, defeating the point of a practice loop. The maintainer considered and declined the smaller alternative — remembering the position for the next manual Play — since the screen-off case is exactly the one a manual Play is inconvenient for. Best-effort throughout: nothing here promises a mobile browser will keep a backgrounded audio session alive, which is why the unrecoverable fallback (Case /2) is kept rather than assumed away. **Found and fixed while auditing this task's own uncommitted diff, before it had ever been committed:** the original `_handleResume` re-anchored `origin` against `ctx.currentTime` after unconditionally taking whatever `resume()` returned, including a *replacement* context (`context.js`'s STUCK_STATES path, AC-4.1.10/2) — but a replacement's clock starts fresh, unrelated to the suspended context's, so `recoverFromStall`'s stall-shift math (built to compare two readings of the *same* clock) silently produced a nonsense origin instead of erroring: the run would hang, scheduled hundreds of seconds into a clock that had barely started, rather than falling back to the documented reset-to-top. AC-4.1.6/2 was broadened to cover a replaced context explicitly, and `_handleResume` now checks the returned context is the *same object* it suspended before continuing in place, falling back to `stop()` otherwise.
- [X] T309 **[test]** Tests for T308 — `tests/e2e/playback.spec.js` (AC-4.1.5 and AC-4.1.6's two prior tests rewritten to the revised wording — a recoverable suspension pauses and, on the tab returning to visible, resumes on its own from the same loop and position, with the loop counter never dropping; AC-4.1.11/1 named test: `navigator.mediaSession.playbackState` and `.metadata.title` observed across a start, a suspend, a resume and a stop, in a Chromium build that exposes the API; AC-4.1.11/2 named test, DOM-proven per T6 — `Navigator.prototype.mediaSession` deleted before the app's own script runs, then a full start/suspend/resume/stop cycle driven for real with nothing there to call, asserting no `pageerror` and ordinary playback throughout), `tests/unit/audio/scheduler.test.js` (`FakeAudioContext` gains a real `statechange` listener registry, a `close()`, and an `allContextsStuck` seam so a test can make every context it or `context.js`'s replace-and-close path constructs stay stuck; tests for AC-4.1.6/1 — a suspend then a `statechange` back to `'running'` continues scheduling further events past where it paused, with no jump backward — and two for AC-4.1.6/2 — a stuck context's suspend never comes back, and a *replaced* context handed back running still falls back rather than scheduling against a mismatched clock, both firing `onStop` exactly as an ordinary stop would), `tests/unit/audio/mediaSession.test.js` (new: AC-4.1.11/2 named test — every export called with no `navigator` global present, the actual shape of this Vitest environment, throws nothing and changes nothing observable), `tests/e2e/harmony.spec.js` (AC-2.10.2/1 and AC-2.10.2/3 gain DOM-reaching named tests per T6 — see T311; their titles cross-reference AC-4.1.6/1 and AC-2.7.2/x only from the *dated parenthetical* below AC-2.10.2's Cases, never inside a Case's own title, after auditing found that an embedded cross-reference inside a Case title makes `check:trace`'s per-mention scan treat the *other* AC as tested by a wrongly-named test — a false HIGH finding against an already-proven criterion). Verified red against the unfixed code: the old AC-4.1.5/4.1.6 tests failed once the wording and the reset-on-suspend behaviour changed underneath them, confirming they were exercising the exact clause being revised; the replaced-context regression test failed against the original `_handleResume` before its fix.
- [X] T310 **[new capability]** Cycle through chord progressions while practising — `specs/001-rhythm-master-mvp/spec.md` (User Story 45 added: AC-2.10.1–AC-2.10.3 with Cases and a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-048 added), `src/core/harmony.js` (`progressionIndexOf`, `progressionIndexFor`, `withProgression` — the progression-catalogue mirror of `fillIndexOf`/`fillIndexFor`/`withArpeggio`, matched from the Pattern's own chords via the existing `matchProgression` rather than a stored id), `src/main.js` (`state.progressionCycle`, the same shape as `state.fillCycle` and independent of it; `progressionCycling`/`progressionIndexAt`/`progressionIdAt`; `playingAt` substitutes the progression in force before the fill in force, so the two overlays compose; `rebaseProgressionCycle`; `onProgressionCycle` handler; `onFillCycleRepeats` rebases whichever of the two cycles is on, since they share one Repeats setting; the transport's `onLoop` advances either catalogue independently at its own boundary, and `onStop`/`onPlay` reset `progressionCycle` to its starting place exactly as `fillCycle` already does; `onPlaySong` stands progression cycling down too, mirroring Cycle fills' exclusivity with Play song; `window.__rm.progressionInForce` test seam added beside `fillInForce`), `src/ui/controls.js` (`renderFillCycle` gains a second, independent `.progression-cycle` toggle beside `.fill-cycle`, sharing the row's one Repeats box). Implements AC-2.10.1/1–/5, AC-2.10.2/1–/6, AC-2.10.3/1–/2. New plan item P-048.
- [X] T311 **[test]** Tests for T310 — `tests/unit/core/harmony.test.js` (three named tests over the pure core: AC-2.10.2/1's period math and the two catalogues' independence on a shared `repeats`, AC-2.10.2/3's full-catalogue wrap in declared group order, AC-2.10.1/3's `progressionIndexOf`/`withProgression` matched-vs-unmatched and overlay behaviour), `tests/e2e/harmony.spec.js` (AC-2.10.1/1–/5, AC-2.10.2/1–/7, AC-2.10.3/1–/2 named tests, DOM-proven — the toggle and its independence from Cycle fills, the shared Repeats box, the picker/note-bands/pitch-strip/score following the progression in force while the real transport cycles it, MIDI export keeping the Pattern's own progression, and — added to satisfy T6, since AC-2.10.2/1 and AC-2.10.2/3 mention no control or view themselves but sit under a parent AC-2.10.2 whose own added parenthetical happens to say "picker" — DOM-driven proofs of the same period math and catalogue-order wrap the core tests already prove, through the real transport with a qualifier appended after each Case's full title). Verified red against the unimplemented capability.
- [X] T312 **[spec defect + bug]** The library search field keeps focus while you type — `specs/001-rhythm-master-mvp/spec.md` (AC-5.2.4 added, with a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-025's AC range extended to AC-5.2.4; T312/T313 added to its task columns), `src/ui/library.js` (`renderLibrary` carries the existing `.library-search` node across renders instead of rebuilding it: the input is looked up before `innerHTML = ''`, its focus state and selection range are captured, the `input` listener is attached once and dispatches through `search.rmOnSearch` so a per-render `handlers` object cannot go stale, the value is written only when it differs — an identical assignment still collapses the caret to the end — and focus plus the selection are restored after re-appending, since moving a node blurs it). Implements AC-5.2.4. Reported by the maintainer: every keystroke re-rendered the library and replaced the input, so focus was lost after each character and the field had to be clicked again. Classified as a spec defect first: no AC asserted that the field survives the list updating, so the AC was written before the fix.
- [X] T313 **[test]** Test for T312 — `tests/e2e/library.spec.js` (AC-5.2.4 named test, e2e per T6: the field is clicked, then "bossa" is typed one character at a time through the real keyboard — `fill()` writes the whole string in a single event and would never have reproduced the bug — asserting the field is still focused after every keystroke, ends holding "bossa" with the caret at the end, and that the list has narrowed). Verified red against the unfixed code: focus was lost on the first character.
- [X] T314 **[data]** A condensed twin of every exercise Condense can simplify — `data/seed-patterns.json` (122 Patterns appended, taking the shipped library from 305 to 427: every `Exercises` Pattern for which `canCondense` is true, percussive and melodic alike, in exactly the form `src/core/condense.js` produces, so pressing Condense on the original gives this Pattern's rhythm). Asked for by the maintainer 2026-09-19: "I am using the condensed feature on a lot of these exercises, so I would love to have a condensed tag and I would like you to auto condense any of the exercises that can be condensed, just the exercises … and then create those with a condensed tag and include the other tags as well." 61 percussive and 61 melodic; fingerprinted against the whole shipped library and against each other before writing — no duplicates, and all 427 names unique. **Naming and Tags are scaled, not copied.** A halving takes each note value one step finer, so the note-value words in the name and in the material Tags are scaled by the number of halvings the Pattern took (`Quarter Notes — Rest on Four` → `Sixteenth Notes — Rest on Four (Condensed)`; `Eighths and Quarters — Rest on Two` → `Sixteenths and Eighths — Rest on Two (Condensed)`), since the maintainer's objection was exactly that the old names no longer described the rhythm: "look at a lot of the names you have there, things like quarter note or eighth note … that is no longer a good name for them." Level (`Beginner`/`Intermediate`/`Advanced`), meter (`2/4`, `Mixed Meter` — Condense preserves every Time Signature) and `Offbeats` carry across unchanged, plus `Condensed`. `percussive`, `melodic` and `built-in` are **not** stored: they are automatic Tags, computed from the Pattern and from which store it came from (data-model §4), and `validate:seed` rejects a stored one — the maintainer asked for a percussive Tag and it is already there, derived. Two descriptors that named a bar were renamed by hand where two halvings turned that bar into a beat (`One Note Per Bar` → `One Note Per Beat`, `Backbeat Bars` → `Backbeat Shape`); descriptors on the 1-halving Patterns still speak of bars, which halved rather than vanished, and were left alone. **A melodic twin's `harmony.change` becomes `pass`**: the Measures its progression was written over are gone, and the maintainer chose per-pass over per-Measure so the whole progression stays audible — "the chord progression should be set so it changes every pass as opposed to every measure … and then leave the actual progression itself intact". The chords, qualities, arpeggio, Key, scale and Register are untouched.
- [X] T315 **[test]** Tests for T314 — `tests/unit/storage/seed.test.js` (five tests over the shipped data: one condensed twin for every condensable exercise and no other; each is byte-equal in rhythm to what `condense` makes of a shipped exercise, matched by `rhythmFingerprint`, so the file and the Condense control cannot drift; none can be condensed further; each carries a level Tag and stores no automatic Tag; every melodic one advances its progression once a pass). `tools/validate-seed.js` and the existing `AC-16.1.x` tests cover the rest, unchanged and now exercising 122 more Patterns.
- [X] T316 **[new capability]** A library row says whether the Pattern is built-in or your own — `specs/001-rhythm-master-mvp/spec.md` (AC-5.1.7 added with Cases /1–/4 and a dated parenthetical), `specs/001-rhythm-master-mvp/plan.md` (P-024's AC range extended to AC-5.1.7), `src/ui/library.js` (`renderEntry` appends a `.pattern-provenance` marker reading "Built-in" or "Custom", carrying `data-provenance`; not a control — provenance is decided by which store the Pattern came from and nothing can change it from a row), `src/styles/tokens.css` (`.pattern-provenance`, the word carried in every state so colour is never the only signal). Implements AC-5.1.7/1–/4. Extends P-024. Reported by the maintainer 2026-09-19: "I'm really having trouble finding the ones I changed after that … I want to make sure I can easily find the built-in ones versus the ones that are not built-in." Diagnosis: the Tags were never the problem. The `built-in` and `custom` automatic Tags exist, filter correctly, and every shipped Pattern already carries them — and all 97 melodic exercise twins already carry Tags byte-identical to their percussive originals, verified across all 97. What was missing is on the row: T151 deliberately removed Tag chips from library rows because repeating them on 112 rows made the list far longer to scroll, and an edited copy keeps the name of the Pattern it came from (US-7.3), so two rows could be genuinely indistinguishable. Provenance is the one piece of that information a row cannot do without.
- [X] T317 **[test]** Tests for T316 — `tests/e2e/library.spec.js` (AC-5.1.7/1–/4 named tests, DOM-proven: every shipped row marked "Built-in"; a newly created Pattern's row marked "Custom"; the words read with colour removed from the question entirely; and a shipped Pattern and an owned one renamed to the same name told apart by their markers while both sit in the one unified list).
