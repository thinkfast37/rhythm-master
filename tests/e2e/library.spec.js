...
test('AC-5.3.6 — a user Tag can be removed; an automatic one cannot', async ({ page }) => {
  await page.goto('/');
  await page.evaluate(() => {
    const id = window.__rm.seedStore.loadAll()[0].id;
    window.__rm.handlers.onAddTag(id, 'warmup');
  });
  await expect(page.locator('.pattern-item .tag-chip.user', { hasText: 'warmup' }).first()).toBeVisible();

  await page
    .locator('.pattern-item .tag-chip.user', { hasText: 'warmup' })
    .first()
    .locator('.tag-remove')
    .click();
  await expect(page.locator('.pattern-item .tag-chip.user', { hasText: 'warmup' })).toHaveCount(0);
  // The automatic Tag is still there and still has no removal control.
  await expect(page.locator('.pattern-item .tag-chip.automatic').first()).toBeVisible();
});
...
test('AC-5.3.7 — user Tags de-duplicate case-insensitively', async ({ page }) => {
  ...
  const tags = await page.evaluate(() => {
    ...
    window.__rm.handlers.onAddTag(id, 'Warmup');
    window.__rm.handlers.onAddTag(id, 'warmup');
    return window.__rm.overlayStore.forPattern(id).tags;
  });
  // Seed Patterns already carry a user Tag from the legacy category, so the
  // assertion is that the second spelling was rejected — not that only one
  // Tag exists.
...
test('AC-5.1.6 — an owned Pattern joins the library alongside shipped ones', async ({ page }) => {
  await page.goto('/');
